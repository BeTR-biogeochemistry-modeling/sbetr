...
!  if(index(bgc_type,'type1_bgc')/=0)then
    SPVAL_ALLOC(this%decomp_ppools_vr(begc:endc, lbj:ubj, 1:7))         ! from jinyun_rr        -zlyu  
...
!  endif
  ...
  if(index(bgc_type,'type1_bgc')/=0)then                    ! if case from jinyun_rr            -zlyu
  ...
  if(index(bgc_type,'type1_bgc')/=0)return                      !from jinyun_rr            -zlyu
  this%cwdp_col(:) = 0._r8 
  ...
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      ...
      this%domp_col(c)  =   this%domp_col(c) + dz(c,j)*this%domp_vr_col(c,j)       ! not in rzacplsbetr_cmupdated version    -zlyu
    enddo
  enddo
