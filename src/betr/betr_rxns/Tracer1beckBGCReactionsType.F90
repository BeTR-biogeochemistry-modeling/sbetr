module Tracer1beckBGCReactionsType
...
#include "bshr_assert.h"
  ...
  public :: bgc_reaction_tracer1beck_run_type
  ...
  end interface bgc_reaction_tracer1beck_run_type

contains
  !-------------------------------------------------------------------------------
  type(bgc_reaction_tracer1beck_run_type) function constructor()
  !
  ! !DESCRIPTION:
  ! create an object of type bgc_reaction_tracer1beck_run_type.
  ! Right now it is purposely empty
    type(bgc_reaction_tracer1beck_run_type), allocatable :: bgc
    allocate(bgc)
    constructor = bgc
  ...
  type(betr_biogeo_flux_type)      , intent(inout) :: biogeo_flux
   ...
   !----------------------------------------------------------------------
   function conc1(time, D, u, L, z)result(ans)
   !
   !DESCRIPTION
   !Analytic solution with constant concentration point source at the top boundary
   !Reference: Tang et al. (2013), GMD
   !time: time since the start of the model, [s]
   !D: diffusivity, [m2/s]
   !u: advection velocity, [m/s]
   !L: total thickness of the column, [m]
