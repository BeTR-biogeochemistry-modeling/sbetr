        ...
        do jj = 1, ntracers
          ...
          if(jj<= ngwmobile_tracers)then
            ...
            if(is_volatile(jj) .and. (.not. is_h2o(jj)) .and. (.not. is_isotope(jj)))then
               call this%add_hist_var2d (it, num2d, fname=trim(tracername)//'_TRACER_P_GAS_FRAC', units='none', &
                    type2d='levtrc', avgflag='A', long_name='fraction of gas phase contributed by '//trim(tracername))
            endif

            if(is_frozen(jj))then
               call this%add_hist_var2d (it, num2d, fname=trim(tracername)//'_TRACER_CONC_FROZEN', units='mol m-3', &
                    type2d='levtrc', avgflag='A', long_name='frozen phase for tracer '//trim(tracername))
            endif
          endif
