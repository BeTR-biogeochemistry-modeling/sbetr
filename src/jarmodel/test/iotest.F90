...
implicit none
  ...
  varl(1)='tmp1'; unitl(1)='K'; freql(1)='day'
  varl(2)='tmp2'; unitl(2)='K'; freql(2)='month'
  varl(3)='tmp3'; unitl(3)='K'; freql(3)='hour'

  call timer%Init(namelist_buffer='junk data')
  dt=timer%get_step_size()
  call hist%init(varl, unitl, freql, 'iotest')

  do
    tsec=timer%get_cur_time()+dt*0.5_r8
    call anal_tempf(yval(1), tsec)
    yval(2)=yval(1)
    yval(3)=yval(1)
