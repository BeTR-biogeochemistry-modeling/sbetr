...
!
! HISTORY:
! Created by Jinyun Tang, October 24, 2018
! !USES:
  ...
  use fakeParaType            , only : fake_para
  use fakeBGCIndexType        , only : fake_bgc_index_type
  use fakeBGCType             , only : fake_bgc_type
...
implicit none
  ...
  character(len=*), parameter :: mod_filename = &
    __FILE__
  public :: fake_bgc_reaction_type
  type, extends(bgc_reaction_type) :: &
    fake_bgc_reaction_type
  private
    type(fake_bgc_type), pointer :: fake_bgc(:,:)
    type(JarBGC_forc_type), pointer :: fake_forc(:,:)
    type(fake_bgc_index_type) :: fake_bgc_index
    logical :: use_c13
    logical :: use_c14
    logical :: nop_limit
    logical :: non_limit
  ...
  end type fake_bgc_reaction_type

  interface fake_bgc_reaction_type
    module procedure constructor
  end interface fake_bgc_reaction_type

contains
!-------------------------------------------------------------------------------
  type(fake_bgc_reaction_type) function constructor()
  !
  ! !DESCRIPTION:
  ! create an object of type fake_bgc_reaction_type.

  type(fake_bgc_reaction_type), allocatable :: bgc
  allocate(bgc)
  constructor = bgc
...
!-------------------------------------------------------------------------------
  ...
  implicit none
  class(fake_bgc_reaction_type)         , intent(inout)    :: this
...
!-------------------------------------------------------------------------------
  ...
  call this%fake_bgc_index%Init()
  !create the models
  allocate(this%fake_bgc(bounds%begc:bounds%endc,lbj:ubj))
  !create model specific forcing data structure
  allocate(this%fake_forc(bounds%begc:bounds%endc,lbj:ubj))
  !initialize
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      call this%fake_bgc(c,j)%Init(batch_mode, bstatus)
      if(bstatus%check_status())return
      call this%fake_forc(c,j)%Init(this%fake_bgc_index%nstvars)
    enddo
  enddo
...
!-------------------------------------------------------------------------------
  ...
  do fc = 1, num_soilc
    c = filter_soilc(fc)
    tracerboundarycond_vars%tracer_gwdif_concflux_top_col(c,1:2,betrtracer_vars%id_trc_no3x)  = 0._r8       !mol m-3, contant boundary condition, as concentration
    ...
    tracerboundarycond_vars%condc_toplay_col(c,groupid(betrtracer_vars%id_trc_no3x))          = 0._r8       !m/s surface conductance
  enddo
...
!-------------------------------------------------------------------------------
  ...
  nstates = this%fake_bgc_index%nstvars
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      is_surflit=(j<=0)
      !do bgc reaction
      call this%fake_bgc(c,j)%runbgc(is_surflit, dtime, this%fake_forc(c,j), nstates, ystates0, ystatesf, betr_status)
...
!----------------------------------------------------------------------
  ...
  nelm =this%fake_bgc_index%nelms
  do j = lbj, ubj
    do fc = 1, num_soilc
      c = filter_soilc(fc)
      if(j<jtops(c))cycle
...
!------------------------------------------------------------------------------
  ...
  use PlantSoilBGCMod          , only : plant_soilbgc_type
  use fakePlantSoilBGCType    , only : fake_plant_soilbgc_type
...
!------------------------------------------------------------------------------
end module fakeBGCReactionsType
