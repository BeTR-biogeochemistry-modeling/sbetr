...
implicit none
 ...
 !real(r8), parameter :: nitrif_n2o_loss_frac = 0.02_r8  ! fraction of N lost as N2O in nitrification (Parton et al., 2001)
  ...
  subroutine calc_anaerobic_frac(this, o2b, summs_forc, o2_decomp_depth, anaerobic_frac, diffus)
    ...
    associate(                                        &
         ...
         h2osoi_vol =>    summs_forc%h2osoi_vol, &
         air_vol    =>    summs_forc%air_vol   , &
         watsat     =>    summs_forc%watsat    , &
         watfc      =>    summs_forc%watfc     , &
         bsw        =>    summs_forc%bsw       , &
         soilpsi    =>    summs_forc%soilpsi   , &
         o2_g2b     =>    summs_forc%o2_g2b    , &
         cellorg    =>    summs_forc%cellorg     &
      )
      o2g = o2b / o2_g2b
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_nitrif_denitrif_rate(this, summs_forc, decompkf_eca,smin_nh4, smin_no3, &
 ...
 ! !LOCAL VARIABLES:
    ...
    associate(                                         &
         ...
         watsat     =>    summs_forc%watsat     , & !
         h2osoi_vol =>    summs_forc%h2osoi_vol , & !
         h2osoi_liq =>    summs_forc%h2osoi_liq , & !
         finundated =>    summs_forc%finundated , & ! Input: [real(r8) (:)]
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_pot_nitr(this, smin_nh4, summs_forc, decompkf_eca, pot_f_nit_mol_per_sec)
  !DESCRIPTION
  ...
  !---------------------------------------------------------------------------------
  subroutine calc_cascade_matrix(this, summsbgc_index, n2_n2o_ratio_denit, cascade_matrix)
  ...
  associate(                                                &
    primvarid    => summsbgc_index%primvarid            , & !
    ...
    lid_nh4_nit        => summsbgc_index%lid_nh4_nit    , & !
    lid_n2o_nit=> summsbgc_index%lid_n2o_nit              & !
  ...
  !---------------------------------------------------------------------------------
  subroutine run_nitden(this, summsbgc_index,summs_forc, decompkf_eca,&
  ...
  call this%calc_anaerobic_frac(o2b, summs_forc, o2_decomp_depth, anaerobic_frac, diffus)

  !calculate potential denitrification and denitrification
  call this%calc_nitrif_denitrif_rate(summs_forc, decompkf_eca, &
  ...
  call this%calc_cascade_matrix(summsbgc_index, n2_n2o_ratio_denit, cascade_matrix)
  end subroutine run_nitden

end module BgcSummsNitDenType
