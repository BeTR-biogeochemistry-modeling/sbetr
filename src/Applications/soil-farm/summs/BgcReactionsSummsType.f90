...
#include "bshr_assert.h"
  ...
  use BgcSummsType          , only : create_summseca_type, summsbgceca_type
  use BgcSummsForcType      , only : summseca_forc_type, create_summseca_forc_type
  use BetrStatusType        , only : betr_status_type
  use BgcConSummsType       , only : BgcConSumms_type, bgc_con_summs
  ...
  type, public, extends(bgc_reaction_type) :: &
    ...
    type(summseca_forc_type), pointer :: summsforc(:,:)
    !type(BgcConSumms_type), pointer :: bgcsummscon(:,:) !RZA
...
contains
  ...
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      call this%summseca(c,j)%UpdateParas(bgc_con_summs)
    enddo
  enddo
...
!-------------------------------------------------------------------------------
  ...
  if(.not. exit_spinup .and. betr_spinup_state/=0 .and. spinup_stage/=2) then
    do c = bounds%begc, bounds%endc
      dom_scalar(c)=1._r8/bgc_con_summs%spinup_factor(9)
    enddo
  endif

  if(betr_spinup_state/=0)then
    move_scalar(tracers%id_trc_Bm) = bgc_con_summs%spinup_factor(7)
    move_scalar(tracers%id_trc_som) = bgc_con_summs%spinup_factor(8)
    move_scalar(tracers%id_trc_dom)=bgc_con_summs%spinup_factor(9)
  endif
 ...
 !----------------------------------------------------------------------
  ...
  do j = lbj, ubj
    ...
    !affinity parameters
    !decomposers
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_mic= plantNutkinetics%km_decomp_nh4_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minn_no3_mic= plantNutkinetics%km_decomp_no3_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minp_mic=  plantNutkinetics%km_decomp_p_vr_col(c_l,j)

    !nitrifiers and denitrifiers
    this%summseca(c_l,j)%competECA%kaff_minn_nh4_nit= plantNutkinetics%km_nit_nh4_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%kaff_minn_no3_den= plantNutkinetics%km_den_no3_vr_col(c_l,j)
    !mineral surfaces
    ...
    this%summseca(c_l,j)%competECA%compet_bn_mic = plantNutkinetics%decomp_eff_ncompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bp_mic = plantNutkinetics%decomp_eff_pcompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bn_den = plantNutkinetics%den_eff_ncompet_b_vr_col(c_l,j)
    this%summseca(c_l,j)%competECA%compet_bn_nit = plantNutkinetics%nit_eff_ncompet_b_vr_col(c_l,j)
  ...
  subroutine Init_betrbgc(this, bounds, lbj, ubj, betrtracer_vars, namelist_buffer, bstatus)
    ...
    call this%summsbgc_index%Init(bgc_con_summs%use_c13, bgc_con_summs%use_c14, &
       bgc_con_summs%non_limit, bgc_con_summs%nop_limit, betr_maxpatch_pft)
    ...
    do j = lbj, ubj
      do c = bounds%begc, bounds%endc
        ...
        call this%summsforc(c,j)%Init(this%summsbgc_index)
      enddo
    enddo
    this%use_c13 = bgc_con_summs%use_c13
    this%use_c14 = bgc_con_summs%use_c14
    this%nop_limit=bgc_con_summs%nop_limit
    this%non_limit=bgc_con_summs%non_limit
  ...
  subroutine set_boundary_conditions(this, bounds, num_soilc, filter_soilc, dz_top, betrtracer_vars, &
    ...
    associate(                                       &
      ...
      do fc = 1, num_soilc
         ...
         tracerboundarycond_vars%tracer_gwdif_concflux_top_col(c,1:2,betrtracer_vars%id_trc_dom)  = 0._r8
         tracerboundarycond_vars%tracer_gwdif_concflux_top_col(c,1:2,betrtracer_vars%id_trc_Bm)   = 0._r8
  ...
  subroutine calc_bgc_reaction(this, bounds, col, lbj, ubj, num_soilc, filter_soilc, &
    ...
    do j = lbj, ubj
      do fc = 1, num_soilc
        ...
        if(j<jtops(c))cycle
        is_surf=(j<=0)
        this%summsforc(c,j)%debug=betrtracer_vars%debug .and. j==2
        ...
        call this%summseca(c,j)%runbgc(is_surf, dtime, this%summsforc(c,j),nstates, &
  ...
  !!commenting out fire below as there is no fire yet in BeTR_biogeoFluxType
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      !this%summsforc(c,j)%cflx_output_litr_met= biophysforc%c12flx%cflx_output_litr_met_vr_col(c,j)
      !this%summsforc(c,j)%cflx_output_litr_cel= biophysforc%c12flx%cflx_output_litr_cel_vr_col(c,j)
      !this%summsforc(c,j)%cflx_output_litr_lig= biophysforc%c12flx%cflx_output_litr_lig_vr_col(c,j)
      !this%summsforc(c,j)%cflx_output_litr_cwd= biophysforc%c12flx%cflx_output_litr_cwd_vr_col(c,j)
      !this%summsforc(c,j)%cflx_output_litr_fwd= biophysforc%c12flx%cflx_output_litr_fwd_vr_col(c,j)
      !this%summsforc(c,j)%cflx_output_litr_lwd= biophysforc%c12flx%cflx_output_litr_lwd_vr_col(c,j)

      !this%summsforc(c,j)%nflx_output_litr_met= biophysforc%n14flx%nflx_output_litr_met_vr_col(c,j)
      !this%summsforc(c,j)%nflx_output_litr_cel= biophysforc%n14flx%nflx_output_litr_cel_vr_col(c,j)
      !this%summsforc(c,j)%nflx_output_litr_lig= biophysforc%n14flx%nflx_output_litr_lig_vr_col(c,j)
      !this%summsforc(c,j)%nflx_output_litr_cwd= biophysforc%n14flx%nflx_output_litr_cwd_vr_col(c,j)
      !this%summsforc(c,j)%nflx_output_litr_fwd= biophysforc%n14flx%nflx_output_litr_fwd_vr_col(c,j)
      !this%summsforc(c,j)%nflx_output_litr_lwd= biophysforc%n14flx%nflx_output_litr_lwd_vr_col(c,j)

      !this%summsforc(c,j)%pflx_output_litr_met= biophysforc%p31flx%pflx_output_litr_met_vr_col(c,j)
      !this%summsforc(c,j)%pflx_output_litr_cel= biophysforc%p31flx%pflx_output_litr_cel_vr_col(c,j)
      !this%summsforc(c,j)%pflx_output_litr_lig= biophysforc%p31flx%pflx_output_litr_lig_vr_col(c,j)
      !this%summsforc(c,j)%pflx_output_litr_cwd= biophysforc%p31flx%pflx_output_litr_cwd_vr_col(c,j)
      !this%summsforc(c,j)%pflx_output_litr_fwd= biophysforc%p31flx%pflx_output_litr_fwd_vr_col(c,j)
      !this%summsforc(c,j)%pflx_output_litr_lwd= biophysforc%p31flx%pflx_output_litr_lwd_vr_col(c,j)
  ...
  associate(                              &
    ...
    ncentpools => this%summsbgc_index%nom_pools, &
    is_cenpool_som => this%summsbgc_index%is_cenpool_som &
  )
  do jj = 1, ncentpools
    kc = (jj-1) * nelms + c_loc
    kn = (jj-1) * nelms + n_loc
...
! was commented because missing from BeTR_biogeoFluxType
      !biogeo_flux
   ...
   do j = lbj, ubj
     do fc = 1, num_soilc
        ...
        do kk = betrtracer_vars%id_trc_beg_dom, betrtracer_vars%id_trc_end_dom, nelm
          ...
          biogeo_state%c12state_vars%monoc_vr_col(c,j) = &
            catomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c_loc)

          biogeo_state%n14state_vars%monon_vr_col(c,j) =  &
            natomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+n_loc)

          biogeo_state%p31state_vars%monop_vr_col(c,j) =  &
            patomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+p_loc)

          if(this%use_c13)then
            biogeo_state%c13state_vars%monoc_vr_col(c,j) =  &
              c13atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c13_loc)
          endif

          if(this%use_c14)then
            biogeo_state%c14state_vars%monoc_vr_col(c,j) =  &
              c14atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c14_loc)
          endif
        ...
        do kk = betrtracer_vars%id_trc_beg_Bm, betrtracer_vars%id_trc_end_Bm, nelm
          ...
          biogeo_state%c12state_vars%polyc_vr_col(c,j) =  &
            catomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c_loc)

          biogeo_state%n14state_vars%polyn_vr_col(c,j) =  &
            natomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+n_loc)

          biogeo_state%p31state_vars%polyp_vr_col(c,j) =  &
            patomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+p_loc)

          if(this%use_c13)then
            biogeo_state%c13state_vars%polyc_vr_col(c,j) = &
              c13atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c13_loc)
          endif

          if(this%use_c14)then
            biogeo_state%c14state_vars%polyc_vr_col(c,j) =  &
              c14atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c14_loc)
          endif
        enddo
        ...
        do kk = betrtracer_vars%id_trc_beg_som, betrtracer_vars%id_trc_end_som, nelm
          ...
          biogeo_state%c12state_vars%micc_vr_col(c,j) =  &
            catomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c_loc)

          biogeo_state%n14state_vars%micn_vr_col(c,j) =  &
            natomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+n_loc)

          biogeo_state%p31state_vars%micp_vr_col(c,j) =  &
            patomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+p_loc)

          if(this%use_c13)then
            biogeo_state%c13state_vars%micc_vr_col(c,j) =  &
              c13atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c13_loc)
          endif

          if(this%use_c14)then
            biogeo_state%c14state_vars%micc_vr_col(c,j) =  &
              c14atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c14_loc)
          endif   

          biogeo_state%c12state_vars%enzc_vr_col(c,j) =  &
            catomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c_loc)

          biogeo_state%n14state_vars%enzn_vr_col(c,j) =  &
            natomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+n_loc)

          biogeo_state%p31state_vars%enzp_vr_col(c,j) =  &
            patomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+p_loc)

          if(this%use_c13)then
            biogeo_state%c13state_vars%enzc_vr_col(c,j) =  &
              c13atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c13_loc)
          endif

          if(this%use_c14)then
            biogeo_state%c14state_vars%enzc_vr_col(c,j) =  &
              c14atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c14_loc)
          endif
        
          biogeo_state%c12state_vars%resc_vr_col(c,j) =  &
            catomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c_loc)

          biogeo_state%n14state_vars%resn_vr_col(c,j) =  &
            natomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+n_loc)

          biogeo_state%p31state_vars%resp_vr_col(c,j) =  &
            patomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+p_loc)

          if(this%use_c13)then
            biogeo_state%c13state_vars%resc_vr_col(c,j) =  &
              c13atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c13_loc)
          endif

          if(this%use_c14)then
            biogeo_state%c14state_vars%resc_vr_col(c,j) =  &
              c14atomw * tracerstate_vars%tracer_conc_mobile_col(c, j, kk-1+c14_loc)
          endif
        enddo
