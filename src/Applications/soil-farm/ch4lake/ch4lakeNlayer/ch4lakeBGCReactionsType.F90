module ch4lakeBGCReactionsType
...
#include "bshr_assert.h"
  ...
  use ch4lakeBGCType         , only : ch4lake_bgc_type
  use JarBgcForcType        , only : JarBGC_forc_type
  use BetrStatusType        , only : betr_status_type
  use ch4lakeBGCIndexType    , only : ch4lake_bgc_index_type
  use ch4lakeParaType        , only : ch4lake_para
  implicit none
  ...
  end type ch4lake_bgc_reaction_type

  interface ch4lake_bgc_reaction_type
     module procedure constructor
  end interface ch4lake_bgc_reaction_type

contains

  !-------------------------------------------------------------------------------
  type(ch4lake_bgc_reaction_type) function constructor()
    !
    ! ! DESCRIPTION:
    !
    ! create an object of type ch4lake_bgc_reaction_type.
    ! Right now it is purposely empty
   type(ch4lake_bgc_reaction_type), allocatable :: bgc
  ...
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      call this%ch4lake(c,j)%UpdateParas(ch4lake_para, bstatus)
  ...
  associate(                                                           &
   ...
   lat                     => biophysforc%lat                        , &
   nelm                    => this%ch4lake_bgc_index%nelms            , &
   c_loc                   => this%ch4lake_bgc_index%c_loc            , &
   n_loc                   => this%ch4lake_bgc_index%n_loc            , &
    ...
    do j = lbj, ubj
      do c = bounds%begc, bounds%endc
        ...
        call rescale_tracer_group(c, j, tracers%id_trc_beg_Bm, &
             tracers%id_trc_end_Bm, nelm, 1._r8/(ch4lake_para%spinup_factor(7)))

        call rescale_tracer_group(c, j, tracers%id_trc_beg_som, &
             tracers%id_trc_end_som, nelm, 1._r8/(ch4lake_para%spinup_factor(8)*latacc))

        call rescale_tracer_group(c, j, tracers%id_trc_beg_pom, &
             tracers%id_trc_end_pom, nelm, 1._r8/(ch4lake_para%spinup_factor(9)*latacc))
      enddo
    enddo
  endif
  if(exit_spinup)then
    ...
    do j = lbj, ubj
      do c = bounds%begc, bounds%endc
        !som1
        call rescale_tracer_group(c, j, tracers%id_trc_beg_Bm, &
             tracers%id_trc_end_Bm, nelm, ch4lake_para%spinup_factor(7))

        call rescale_tracer_group(c, j, tracers%id_trc_beg_som, &
             tracers%id_trc_end_som, nelm, ch4lake_para%spinup_factor(8)*latacc)

        call rescale_tracer_group(c, j, tracers%id_trc_beg_pom, &
             tracers%id_trc_end_pom, nelm, ch4lake_para%spinup_factor(9)*latacc)
  ...
  call calc_P_weathering_flux(bounds, biophysforc, ch4lake_para, P_weather_flx)
  ...
  do j = lbj, ubj
    do p = 1, nactpft
      this%ch4lake(c_l,j)%competECA%mumax_minn_nh4_plant(p) = plantNutkinetics%plant_nh4_vmax_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%mumax_minn_no3_plant(p) = plantNutkinetics%plant_no3_vmax_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%mumax_minp_plant(p) = plantNutkinetics%plant_p_vmax_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%kaff_minn_no3_plant(p)= plantNutkinetics%plant_no3_km_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%kaff_minn_nh4_plant(p)= plantNutkinetics%plant_nh4_km_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%kaff_minp_plant(p)   = plantNutkinetics%plant_p_km_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%plant_froot_nn(p) = plantNutkinetics%plant_eff_ncompet_b_vr_patch(p,j)
      this%ch4lake(c_l,j)%competECA%plant_froot_np(p) = plantNutkinetics%plant_eff_pcompet_b_vr_patch(p,j)
    enddo
    if(lbcalib)then
      this%ch4lake(c_l,j)%competECA%kaff_minn_nh4_mic =plantNutkinetics%km_decomp_nh4_vr_col(c_l,j)
      this%ch4lake(c_l,j)%competECA%kaff_minn_no3_mic =plantNutkinetics%km_decomp_no3_vr_col(c_l,j)
      this%ch4lake(c_l,j)%competECA%kaff_minp_mic = plantNutkinetics%km_decomp_p_vr_col(c_l,j)
      this%ch4lake(c_l,j)%competECA%kaff_minn_nh4_nit = plantNutkinetics%km_nit_nh4_vr_col(c_l,j)
      this%ch4lake(c_l,j)%competECA%kaff_minn_no3_den = plantNutkinetics%km_den_no3_vr_col(c_l,j)
    endif
    !mineral surfaces
    this%ch4lake(c_l,j)%competECA%kaff_minn_nh4_msurf= plantNutkinetics%km_minsurf_nh4_vr_col(c_l,j)   !this is ignored at this moment
    this%ch4lake(c_l,j)%competECA%kaff_minp_msurf= plantNutkinetics%km_minsurf_p_vr_col(c_l,j)

    !effective p competing decomposers

    this%ch4lake_forc(c_l,j)%msurf_nh4 = plantNutkinetics%minsurf_nh4_compet_vr_col(c_l,j)   !this  number needs update
    this%ch4lake_forc(c_l,j)%msurf_minp= plantNutkinetics%minsurf_p_compet_vr_col(c_l,j)    !this  number needs update
  ...
  subroutine Init_betrbgc(this, bounds, lbj, ubj, betrtracer_vars, namelist_buffer, bstatus)
    ...
    call this%ch4lake_bgc_index%Init(ch4lake_para%use_c13, ch4lake_para%use_c14, &
       ch4lake_para%non_limit, ch4lake_para%nop_limit, betr_maxpatch_pft)

    if(bstatus%check_status())return

    !create the models
    allocate(this%ch4lake(bounds%begc:bounds%endc,lbj:ubj))

    !create model specific forcing data structure
    allocate(this%ch4lake_forc(bounds%begc:bounds%endc,lbj:ubj))

    !initialize
    do j = lbj, ubj
      do c = bounds%begc, bounds%endc
        call this%ch4lake(c,j)%Init(ch4lake_para, batch_mode, bstatus)
        if(bstatus%check_status())return

        call this%ch4lake_forc(c,j)%Init(this%ch4lake_bgc_index%nstvars)
      enddo
    enddo
    this%use_c13 = ch4lake_para%use_c13
    this%use_c14 = ch4lake_para%use_c14
    this%nop_limit=ch4lake_para%nop_limit
    this%non_limit=ch4lake_para%non_limit
  ...
  subroutine calc_bgc_reaction(this, bounds, col, lbj, ubj, num_soilc, filter_soilc, &
    ...
    nstates = this%ch4lake_bgc_index%nstvars
    ...
    type is(ch4lake_plant_soilbgc_type)
    ...
    do j = lbj, ubj
      do fc = 1, num_soilc
        ...
        if(j<jtops(c))cycle
        is_surflit=(j<=0)
        this%ch4lake_forc(c,j)%debug=betrtracer_vars%debug
        this%ch4lake(c,j)%bgc_on=.not. betrtracer_vars%debug
...
!        if(.not. betrtracer_vars%debug)then
          ...
          call this%rm_ext_output(c, j, dtime, nstates, ystatesf, this%ch4lake_bgc_index,&
             this%ch4lake_forc(c,j), biogeo_flux)
!        endif
        call this%precision_filter(nstates, ystatesf)
        this%ch4lake_bgc_index%debug=betrtracer_vars%debug
...
!        betrtracer_vars, tracerstate_vars,  'after bgcreact',betr_status)
  ...
  !--------------------------------------------------------------------
  subroutine rm_ext_output(this, c, j, dtime, nstates, ystatesf, ch4lake_bgc_index, ch4lake_forc, biogeo_flux)
  !
  ! DESCRIPTION
  ...
  type(ch4lake_bgc_index_type) , intent(in) :: ch4lake_bgc_index
  type(JarBGC_forc_type)      , intent(in) :: ch4lake_forc
  type(betr_biogeo_flux_type) , intent(inout) :: biogeo_flux
  ...
  integer :: loc_indx(3)
  associate(                         &
    lit1 =>  ch4lake_bgc_index%lit1 , &
    lit2 =>  ch4lake_bgc_index%lit2 , &
    lit3 =>  ch4lake_bgc_index%lit3 , &
    cwd =>  ch4lake_bgc_index%cwd   , &
    lwd =>  ch4lake_bgc_index%lwd   , &
    fwd =>  ch4lake_bgc_index%fwd   , &
    c13_loc=>  ch4lake_bgc_index%c13_loc,&
    c14_loc=>  ch4lake_bgc_index%c14_loc,&
    c_loc=>  ch4lake_bgc_index%c_loc,&
    n_loc=>  ch4lake_bgc_index%n_loc,&
    p_loc=>  ch4lake_bgc_index%p_loc,&
    som1 =>  ch4lake_bgc_index%som1 , &
    som2 =>  ch4lake_bgc_index%som2 , &
    som3 =>  ch4lake_bgc_index%som3 , &
    nelms => ch4lake_bgc_index%nelms, &
    frac_loss_lit_to_fire => ch4lake_forc%frac_loss_lit_to_fire, &
    frac_loss_cwd_to_fire => ch4lake_forc%frac_loss_cwd_to_fire, &
  ...
  associate( &
     litr_beg =>  this%ch4lake_bgc_index%litr_beg  , &
     litr_end =>  this%ch4lake_bgc_index%litr_end  , &
     wood_beg =>  this%ch4lake_bgc_index%wood_beg  , &
     wood_end =>  this%ch4lake_bgc_index%wood_end  , &
     som_beg =>  this%ch4lake_bgc_index%som_beg    , &
     som_end =>  this%ch4lake_bgc_index%som_end    , &
     dom_beg =>  this%ch4lake_bgc_index%dom_beg    , &
     dom_end =>  this%ch4lake_bgc_index%dom_end    , &
     pom_beg =>  this%ch4lake_bgc_index%pom_beg    , &
     pom_end =>  this%ch4lake_bgc_index%pom_end    , &
     Bm_beg  =>  this%ch4lake_bgc_index%Bm_beg     , &
     Bm_end  =>  this%ch4lake_bgc_index%Bm_end     , &
     groupid =>  betrtracer_vars%groupid            &
  )
  call betr_status%reset()
  SHR_ASSERT_ALL((ubound(jtops) == (/bounds%endc/)), errMsg(mod_filename,__LINE__),betr_status)
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      c = filter_soilc(fc)
      if(j<jtops(c))cycle
      this%ch4lake_forc(c,j)%latacc = latacc(c)
      this%ch4lake_forc(c,j)%plant_ntypes = this%nactpft
      this%ch4lake_forc(c,j)%ystates(:) = 0._r8


      !litter
      this%ch4lake_forc(c,j)%ystates(litr_beg:litr_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_litr:betrtracer_vars%id_trc_end_litr)

      !wood
      this%ch4lake_forc(c,j)%ystates(wood_beg:wood_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_wood:betrtracer_vars%id_trc_end_wood)

      !som
      this%ch4lake_forc(c,j)%ystates(som_beg:som_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_som:betrtracer_vars%id_trc_end_som)
      if(this%ch4lake_forc(c,j)%ystates(som_beg)<=tiny_cval)this%ch4lake_forc(c,j)%ystates(som_beg:som_end)=0._r8

      !dom
      !this%ch4lake_forc(c,j)%ystates(dom_beg:dom_end)= &
      !    tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_dom:betrtracer_vars%id_trc_end_dom)
      !if(this%ch4lake_forc(c,j)%ystates(dom_beg)<=tiny_cval)this%ch4lake_forc(c,j)%ystates(dom_beg:dom_end)=0._r8

      this%ch4lake_forc(c,j)%ystates(pom_beg:pom_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_pom:betrtracer_vars%id_trc_end_pom)
      if(this%ch4lake_forc(c,j)%ystates(pom_beg)<=tiny_cval)this%ch4lake_forc(c,j)%ystates(pom_beg:pom_end)=0._r8

      !microbial biomass
      this%ch4lake_forc(c,j)%ystates(Bm_beg:Bm_end)= &
          tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_Bm:betrtracer_vars%id_trc_end_Bm)

      if(this%ch4lake_forc(c,j)%ystates(Bm_beg)<=tiny_cval)this%ch4lake_forc(c,j)%ystates(Bm_beg:Bm_end)=0._r8

      !non-soluble phase of mineral p
      k1= betrtracer_vars%id_trc_beg_minp; k2 = this%ch4lake_bgc_index%lid_minp_secondary
      this%ch4lake_forc(c,j)%ystates(k2) = fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,k1))

      k1 = betrtracer_vars%id_trc_end_minp;   k2 = this%ch4lake_bgc_index%lid_minp_occlude
      this%ch4lake_forc(c,j)%ystates(k2) = fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,k1))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_n2) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_o2) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_o2))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_ar) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_ar))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_co2)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_co2x))

      if(this%use_c13)then
        this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_c13_co2)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_c13_co2x))
      endif
      if(this%use_c14)then
        this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_c14_co2)= &
          fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_c14_co2x))
      endif

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_ch4)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_ch4))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_nh4)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_nh3x))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_no3)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_no3x))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_n2o)= &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2o))

      this%ch4lake_forc(c,j)%ystates(this%ch4lake_bgc_index%lid_minp_soluble) = &
           fpmax(tracerstate_vars%tracer_conc_mobile_col(c,j,betrtracer_vars%id_trc_p_sol))

      !input
      this%ch4lake_forc(c,j)%cflx_input_litr_met = biophysforc%c12flx%cflx_input_litr_met_vr_col(c,j)
      this%ch4lake_forc(c,j)%cflx_input_litr_cel = biophysforc%c12flx%cflx_input_litr_cel_vr_col(c,j)
      this%ch4lake_forc(c,j)%cflx_input_litr_lig = biophysforc%c12flx%cflx_input_litr_lig_vr_col(c,j)
      this%ch4lake_forc(c,j)%cflx_input_litr_cwd = biophysforc%c12flx%cflx_input_litr_cwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%cflx_input_litr_lwd = biophysforc%c12flx%cflx_input_litr_lwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%cflx_input_litr_fwd = biophysforc%c12flx%cflx_input_litr_fwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_met = biophysforc%n14flx%nflx_input_litr_met_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_cel = biophysforc%n14flx%nflx_input_litr_cel_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_lig = biophysforc%n14flx%nflx_input_litr_lig_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_cwd = biophysforc%n14flx%nflx_input_litr_cwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_lwd = biophysforc%n14flx%nflx_input_litr_lwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%nflx_input_litr_fwd = biophysforc%n14flx%nflx_input_litr_fwd_vr_col(c,j)

      this%ch4lake_forc(c,j)%pflx_input_litr_met = biophysforc%p31flx%pflx_input_litr_met_vr_col(c,j)
      this%ch4lake_forc(c,j)%pflx_input_litr_cel = biophysforc%p31flx%pflx_input_litr_cel_vr_col(c,j)
      this%ch4lake_forc(c,j)%pflx_input_litr_lig = biophysforc%p31flx%pflx_input_litr_lig_vr_col(c,j)
      this%ch4lake_forc(c,j)%pflx_input_litr_cwd = biophysforc%p31flx%pflx_input_litr_cwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%pflx_input_litr_fwd = biophysforc%p31flx%pflx_input_litr_fwd_vr_col(c,j)
      this%ch4lake_forc(c,j)%pflx_input_litr_lwd = biophysforc%p31flx%pflx_input_litr_lwd_vr_col(c,j)

      !mineral nutrient input
      this%ch4lake_forc(c,j)%sflx_minn_input_nh4 = biophysforc%n14flx%nflx_minn_input_nh4_vr_col(c,j)     !nh4 from deposition and fertilization
      this%ch4lake_forc(c,j)%sflx_minn_input_no3 = biophysforc%n14flx%nflx_minn_input_no3_vr_col(c,j)
      this%ch4lake_forc(c,j)%sflx_minn_nh4_fix_nomic = biophysforc%n14flx%nflx_minn_nh4_fix_nomic_vr_col(c,j)       !nh4 from fixation
      this%ch4lake_forc(c,j)%sflx_minp_input_po4 = biophysforc%p31flx%pflx_minp_input_po4_vr_col(c,j)     !inorganic P from deposition and fertilization
      this%ch4lake_forc(c,j)%sflx_minp_weathering_po4 = biophysforc%p31flx%pflx_minp_weathering_po4_vr_col(c,j)
      this%ch4lake_forc(c,j)%biochem_pmin = biophysforc%biochem_pmin_vr(c,j)

      !burning fraction
      this%ch4lake_forc(c,j)%frac_loss_lit_to_fire = biophysforc%frac_loss_lit_to_fire_col(c)
      this%ch4lake_forc(c,j)%frac_loss_cwd_to_fire = biophysforc%frac_loss_cwd_to_fire_col(c)
      !environmental variables
      this%ch4lake_forc(c,j)%temp   = biophysforc%t_soisno_col(c,j)            !temperature
      this%ch4lake_forc(c,j)%depz   = col%z(c,j)            !depth of the soil
      this%ch4lake_forc(c,j)%dzsoi  = col%dz(c,j)            !soil thickness
      this%ch4lake_forc(c,j)%sucsat  = biophysforc%sucsat_col(c,j)            ! Input:  [real(r8) (:,:)] minimum soil suction [mm]
      this%ch4lake_forc(c,j)%soilpsi = max(biophysforc%smp_l_col(c,j)*grav*1.e-6_r8,-15._r8)    ! Input:  [real(r8) (:,:)] soilwater pontential in each soil layer [MPa]
      this%ch4lake_forc(c,j)%bsw = biophysforc%bsw_col(c,j)
      this%ch4lake_forc(c,j)%bd   = biophysforc%bd_col(c,j)              !bulk density
      this%ch4lake_forc(c,j)%pct_sand = biophysforc%cellsand_col(c,j)
      this%ch4lake_forc(c,j)%pct_clay = biophysforc%cellclay_col(c,j)
      this%ch4lake_forc(c,j)%h2osoi_vol = biophysforc%h2osoi_vol_col(c,j)
      this%ch4lake_forc(c,j)%h2osoi_liq = biophysforc%h2osoi_liq_col(c,j)
      this%ch4lake_forc(c,j)%air_vol = biophysforc%air_vol_col(c,j)
      this%ch4lake_forc(c,j)%finundated = biophysforc%finundated_col(c)
      this%ch4lake_forc(c,j)%watsat = biophysforc%watsat_col(c,j)
      this%ch4lake_forc(c,j)%watfc = biophysforc%watfc_col(c,j)
      this%ch4lake_forc(c,j)%cellorg = biophysforc%cellorg_col(c,j)
      this%ch4lake_forc(c,j)%pH = biophysforc%soil_pH(c,j)

      !conductivity for plant-aided gas transport
      this%ch4lake_forc(c,j)%aren_cond_n2 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_n2))

      this%ch4lake_forc(c,j)%aren_cond_o2 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_o2))

      this%ch4lake_forc(c,j)%aren_cond_n2o = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_n2o))

      this%ch4lake_forc(c,j)%aren_cond_co2 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_co2x))

      if(this%use_c13)then
        this%ch4lake_forc(c,j)%aren_cond_co2_c13 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_c13_co2x))
      endif
      if(this%use_c14)then
        this%ch4lake_forc(c,j)%aren_cond_co2_c14 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_c14_co2x))
      endif
      this%ch4lake_forc(c,j)%aren_cond_ar = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_ar))
      this%ch4lake_forc(c,j)%aren_cond_ch4 = &
          tracercoeff_vars%aere_cond_col(c,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_ch4))

      !phase conversion parameter
      this%ch4lake_forc(c,j)%ch4_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_ch4))
      this%ch4lake_forc(c,j)%co2_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_co2x))
      this%ch4lake_forc(c,j)%o2_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_o2))
      this%ch4lake_forc(c,j)%n2_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_n2))
      this%ch4lake_forc(c,j)%ar_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_ar))
      this%ch4lake_forc(c,j)%n2o_g2b = &
          tracercoeff_vars%gas2bulkcef_mobile_col(c,j,betrtracer_vars%volatilegroupid(betrtracer_vars%id_trc_n2o))
      this%ch4lake_forc(c,j)%o2_w2b = &
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      c = filter_soilc(fc)
      this%ch4lake_forc(c,j)%rt_ar  = biophysforc%c12flx%rt_vr_col(c,j)            !root autotrophic respiration, mol CO2/m3/s
    enddo
  enddo
  end select
  end associate
  ...
  associate(                              &
    nelms   => this%ch4lake_bgc_index%nelms, &
    c_loc   => this%ch4lake_bgc_index%c_loc, &
    n_loc   => this%ch4lake_bgc_index%n_loc, &
    p_loc   => this%ch4lake_bgc_index%p_loc, &
    c13_loc => this%ch4lake_bgc_index%c13_loc, &
    c14_loc => this%ch4lake_bgc_index%c14_loc, &
    lit2    => this%ch4lake_bgc_index%lit2 , &
    lit3    => this%ch4lake_bgc_index%lit3 , &
    ncentpools => this%ch4lake_bgc_index%nom_pools, &
    is_cenpool_som => this%ch4lake_bgc_index%is_cenpool_som &
  )
  do jj = 1, ncentpools
    kc = (jj-1) * nelms + c_loc
    kn = (jj-1) * nelms + n_loc
  ...
  associate(                                                                &
    nom_pools             => this%ch4lake_bgc_index%nom_pools              , & !
    nelms                 => this%ch4lake_bgc_index%nelms                  , & !
    litr_beg              => this%ch4lake_bgc_index%litr_beg               , & !
    litr_end              => this%ch4lake_bgc_index%litr_end               , & !
    wood_beg              => this%ch4lake_bgc_index%wood_beg               , & !
...
!        ystatesf(dom_beg:dom_end)
      ...
      if(.not. fix_ip)then
        ...
        if(this%ch4lake_bgc_index%lid_supp_minp>0)then
            !no P-limitation in this time step
            biogeo_flux%p31flux_vars%supplement_to_sminp_vr_col(c,j) = &
              ystatesf(this%ch4lake_bgc_index%lid_supp_minp)*patomw/dtime
        else
        ...
        ystatesf(this%ch4lake_bgc_index%lid_minp_soluble)

        !fluxes
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_p_sol) =      &
          tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_p_sol) + &
          ystatesf(this%ch4lake_bgc_index%lid_minp_soluble) - &
          ystates0(this%ch4lake_bgc_index%lid_minp_soluble)

        trcid = betrtracer_vars%id_trc_beg_minp
        tracer_flx_netpro_vr(c,j, trcid) = &
          tracer_flx_netpro_vr(c,j, trcid) + &
          ystatesf(this%ch4lake_bgc_index%lid_minp_secondary) - &
          ystates0(this%ch4lake_bgc_index%lid_minp_secondary)

        trcid = betrtracer_vars%id_trc_end_minp
        tracer_flx_netpro_vr(c,j, trcid) =  &
          tracer_flx_netpro_vr(c,j, trcid) + &
          ystatesf(this%ch4lake_bgc_index%lid_minp_occlude) - &
          ystates0(this%ch4lake_bgc_index%lid_minp_occlude)
      endif
      !tracer fluxes
      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_nh3x) =  &
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_nh3x) + &
        ystatesf(this%ch4lake_bgc_index%lid_nh4) - &
        ystates0(this%ch4lake_bgc_index%lid_nh4)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_no3x)  =  &
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_no3x) + &
        ystatesf(this%ch4lake_bgc_index%lid_no3) - &
        ystates0(this%ch4lake_bgc_index%lid_no3)

      tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_o2) ) = &
         ystatesf(this%ch4lake_bgc_index%lid_o2_paere )  - &
         ystates0(this%ch4lake_bgc_index%lid_o2_paere)

      if ( betr_spinup_state == 0 ) then
        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_n2)  ) = &
          ystatesf(this%ch4lake_bgc_index%lid_n2_paere)  - &
          ystates0(this%ch4lake_bgc_index%lid_n2_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_ar)  ) = &
          ystatesf(this%ch4lake_bgc_index%lid_ar_paere)  - &
          ystates0(this%ch4lake_bgc_index%lid_ar_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_co2x)) = &
          ystatesf(this%ch4lake_bgc_index%lid_co2_paere)  - &
          ystates0(this%ch4lake_bgc_index%lid_co2_paere)

        if(this%use_c13)then
          tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c13_co2x)) = &
            ystatesf(this%ch4lake_bgc_index%lid_c13_co2_paere)  - &
            ystates0(this%ch4lake_bgc_index%lid_c13_co2_paere)
        endif

        if(this%use_c14)then
          tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c14_co2x)) = &
            ystatesf(this%ch4lake_bgc_index%lid_c14_co2_paere)  - &
            ystates0(this%ch4lake_bgc_index%lid_c14_co2_paere)
        endif

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_ch4) ) = &
          ystatesf(this%ch4lake_bgc_index%lid_ch4_paere)  - &
          ystates0(this%ch4lake_bgc_index%lid_ch4_paere)

        tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_n2o) ) = &
          ystatesf(this%ch4lake_bgc_index%lid_n2o_paere)  - &
          ystates0(this%ch4lake_bgc_index%lid_n2o_paere)
      endif

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_n2) = &
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_n2) + &
        ystatesf(this%ch4lake_bgc_index%lid_n2) - &
        ystates0(this%ch4lake_bgc_index%lid_n2)

      tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_co2x ) = &
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_co2x ) + &
        ystatesf(this%ch4lake_bgc_index%lid_co2) - &
        ystates0(this%ch4lake_bgc_index%lid_co2)

      if(this%use_c13)then
        tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c13_co2x ) = &
          tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c13_co2x ) + &
          ystatesf(this%ch4lake_bgc_index%lid_c13_co2) - &
          ystates0(this%ch4lake_bgc_index%lid_c13_co2)
      endif
      ...
      biogeo_flux%c12flux_vars%hr_vr_col(c,j) = &
        (ystatesf(this%ch4lake_bgc_index%lid_co2_hr) - &
        ystates0(this%ch4lake_bgc_index%lid_co2_hr))*catomw/dtime

      biogeo_flux%p31flux_vars%secondp_to_occlp_vr_col(c,j) = &
         (ystatesf(this%ch4lake_bgc_index%lid_minp_occlude) - &
          ystates0(this%ch4lake_bgc_index%lid_minp_occlude))*patomw/dtime

      biogeo_flux%n14flux_vars%f_denit_vr_col(c,j)= &
        (ystatesf(this%ch4lake_bgc_index%lid_no3_den) - &
         ystates0(this%ch4lake_bgc_index%lid_no3_den))*natomw/dtime

      biogeo_flux%n14flux_vars%f_nit_vr_col(c,j) = &
        (ystatesf(this%ch4lake_bgc_index%lid_nh4_nit) - &
         ystates0(this%ch4lake_bgc_index%lid_nh4_nit))*natomw/dtime

      biogeo_flux%n14flux_vars%f_n2o_nit_vr_col(c,j) = &
        (ystatesf(this%ch4lake_bgc_index%lid_n2o_nit) - &
         ystates0(this%ch4lake_bgc_index%lid_n2o_nit))*natomw/dtime

      biogeo_flux%p31flux_vars%pflx_minp_weathering_po4_vr_col(c,j) =this%ch4lake_forc(c,j)%sflx_minp_weathering_po4

  select type(plant_soilbgc)
  type is(ch4lake_plant_soilbgc_type)
    do p = 1, this%nactpft
      plant_soilbgc%plant_minn_no3_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minn_no3_pft(p)) - &
          ystates0(this%ch4lake_bgc_index%lid_plant_minn_no3_pft(p)))*natomw/dtime

      plant_soilbgc%plant_minn_nh4_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minn_nh4_pft(p)) - &
          ystates0(this%ch4lake_bgc_index%lid_plant_minn_nh4_pft(p)))*natomw/dtime

      plant_soilbgc%plant_minp_active_yield_flx_vr_patch(p,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minp_pft(p)) - &
           ystates0(this%ch4lake_bgc_index%lid_plant_minp_pft(p)))*patomw/dtime

    enddo
    if(this%nactpft>0)then
      plant_soilbgc%plant_minn_no3_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minn_no3) - &
          ystates0(this%ch4lake_bgc_index%lid_plant_minn_no3))*natomw/dtime

      plant_soilbgc%plant_minn_nh4_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minn_nh4) - &
          ystates0(this%ch4lake_bgc_index%lid_plant_minn_nh4))*natomw/dtime

      plant_soilbgc%plant_minp_active_yield_flx_vr_col(c,j) = &
          (ystatesf(this%ch4lake_bgc_index%lid_plant_minp) - &
           ystates0(this%ch4lake_bgc_index%lid_plant_minp))*patomw/dtime
    endif
