...
#include "bshr_assert.h"
  ...
  use ch4lakeBGCDecompType      , only : DecompCent_type
  use ch4lakeBGCIndexType       , only : ch4lake_bgc_index_type
  use ch4lakeBGCNitDenType      , only : century_nitden_type
  use ch4lakeBGCSOMType         , only : CentSom_type
  use ch4lakeBGCCompetType      , only : Compet_CH4LAKE_type
  ...
  !Note:
  !Keeping ch4lake_bgc_index as a private member is a workaround to call the ode solver
  !it increase the memory for each instance of ch4lake_bgc_type, but enables
  !the ode function to be called by the ode solver

  type, extends(jar_model_type), public :: ch4lake_bgc_type
    type(DecompCent_type),private        :: decompkf_eca
    type(century_nitden_type), private   :: nitden
    type(CentSom_type), private          :: censom
    type(Compet_CH4LAKE_type), public        :: competECA
    type(ch4lake_bgc_index_type), private :: ch4lake_bgc_index
  ...
  contains
    procedure, public  :: init          => init_ch4lake
    procedure, public  :: runbgc        => runbgc_ch4lake
    procedure, public  :: UpdateParas   => UpdateParas_ch4lake
    procedure, public  :: sumup_cnp_msflx => sumup_cnp_msflx_ch4lake
    procedure, public  :: getvarllen    => getvarllen_ch4lake
    procedure, public  :: getvarlist    => getvarlist_ch4lake
    procedure, public  :: init_cold     => init_cold_ch4lake
  ...
  end type ch4lake_bgc_type

  public :: create_jarmodel_ch4lake
contains

  function create_jarmodel_ch4lake()
  ! DESCRIPTION
  ! constructor
    implicit none
    class(ch4lake_bgc_type), pointer :: create_jarmodel_ch4lake
    class(ch4lake_bgc_type), pointer :: bgc

    allocate(bgc)
    create_jarmodel_ch4lake => bgc

  end function create_jarmodel_ch4lake

  !-------------------------------------------------------------------------------
  function getvarllen_ch4lake(this)result(ans)

  implicit none
  class(ch4lake_bgc_type) , intent(inout) :: this
  integer :: ans

  ans = this%ch4lake_bgc_index%nstvars

  end function getvarllen_ch4lake
  !-------------------------------------------------------------------------------
  subroutine getvarlist_ch4lake(this, nstvars, varnames, varunits, vartypes)
  implicit none
  class(ch4lake_bgc_type) , intent(inout) :: this
  ...
  do n = 1, nstvars
    vartypes(n) = this%ch4lake_bgc_index%vartypes(n)
    write(varnames(n),'(A)')trim(this%ch4lake_bgc_index%varnames(n))
    write(varunits(n),'(A)')trim(this%ch4lake_bgc_index%varunits(n))
  enddo
  end subroutine getvarlist_ch4lake
  ...
  !-------------------------------------------------------------------------------
  subroutine UpdateParas_ch4lake(this,  biogeo_con, bstatus)
  ...
  type is(ch4lake_para_type)
    ...
    call this%censom%UpdateParas(this%ch4lake_bgc_index, biogeo_con)
  ...
  end subroutine UpdateParas_ch4lake
  !-------------------------------------------------------------------------------
  subroutine init_ch4lake(this,  biogeo_con,  batch_mode, bstatus)
  ...
  type is(ch4lake_para_type)
    call bstatus%reset()
    call this%ch4lake_bgc_index%Init(biogeo_con%use_c13, biogeo_con%use_c14, &
    ...
    call this%InitAllocate(this%ch4lake_bgc_index)

    call this%censom%Init(this%ch4lake_bgc_index, biogeo_con, bstatus)
  ...
  class default
    call bstatus%reset()
    write(msg,'(A)')'Wrong parameter type passed in for init_ch4lake in ' &
      // errMsg(mod_filename,__LINE__)
    call bstatus%set_msg(msg,err=-1)
    return
  end select
  end subroutine init_ch4lake

  !-------------------------------------------------------------------------------

  subroutine InitAllocate(this, ch4lake_bgc_index)
  use ch4lakeBGCIndexType , only : ch4lake_bgc_index_type
  use betr_varcon         , only : betr_maxpatch_pft, betr_max_soilorder
  implicit none
  class(ch4lake_bgc_type)   , intent(inout) :: this
  ...
  associate(                                   &
    nom_pools=> ch4lake_bgc_index%nom_pools,    &
    nstvars => ch4lake_bgc_index%nstvars  ,     &
    nreactions => ch4lake_bgc_index%nreactions, &
    nprimvars => ch4lake_bgc_index%nprimvars    &
  )
  ...
  subroutine checksum_cascade(this, ch4lake_bgc_index)

  use ch4lakeBGCIndexType       , only : ch4lake_bgc_index_type

  implicit none
  ! !ARGUMENTS:
  class(ch4lake_bgc_type)     , intent(in) :: this
  type(ch4lake_bgc_index_type)   , intent(in) :: ch4lake_bgc_index
  ...
  associate(                                                   &
    ...
    lid_o2    => ch4lake_bgc_index%lid_o2                     , & !
    lid_co2   => ch4lake_bgc_index%lid_co2                    , & !
    lid_nh4   => ch4lake_bgc_index%lid_nh4                    , & !
    lid_c14_co2=> ch4lake_bgc_index%lid_c14_co2               , & !
    lid_c13_co2=> ch4lake_bgc_index%lid_c13_co2               , & !
    lid_co2_hr => ch4lake_bgc_index%lid_co2_hr                , &
    lid_minn_nh4_immob=> ch4lake_bgc_index%lid_minn_nh4_immob , &
    lid_minp_immob => ch4lake_bgc_index%lid_minp_immob        , &
    lid_minp_soluble=> ch4lake_bgc_index%lid_minp_soluble     , &
    lit1_dek_reac => ch4lake_bgc_index%lit1_dek_reac          , &
    lit2_dek_reac => ch4lake_bgc_index%lit2_dek_reac          , &
    lit3_dek_reac => ch4lake_bgc_index%lit3_dek_reac          , &
    som1_dek_reac => ch4lake_bgc_index%som1_dek_reac          , &
    som2_dek_reac => ch4lake_bgc_index%som2_dek_reac          , &
    som3_dek_reac => ch4lake_bgc_index%som3_dek_reac          , &
    cwd_dek_reac => ch4lake_bgc_index%cwd_dek_reac            , &
    lwd_dek_reac => ch4lake_bgc_index%lwd_dek_reac            , &
    fwd_dek_reac => ch4lake_bgc_index%fwd_dek_reac            , &
    lid_n2   => ch4lake_bgc_index%lid_n2                      , & !
    lid_n2o   => ch4lake_bgc_index%lid_n2o                    , & !
    lid_no3   => ch4lake_bgc_index%lid_no3                    , & !
    lid_nh4_nit_reac => ch4lake_bgc_index%lid_nh4_nit_reac    , & !
    lid_no3_den_reac => ch4lake_bgc_index%lid_no3_den_reac      & !
  )
  print*,'checksum'
  ...
  !-------------------------------------------------------------------------------
  subroutine runbgc_ch4lake(this,  is_surflit, dtime, bgc_forc, nstates, ystates0, ystatesf, bstatus)
  ...
  !local variables
  real(r8) :: pot_om_decay_rates(this%ch4lake_bgc_index%nom_pools)
  real(r8) :: pot_co2_hr
  real(r8) :: pot_f_nit_mol_per_sec
  real(r8) :: n2_n2o_ratio_denit
  real(r8) :: yf(this%ch4lake_bgc_index%nstvars)
  ...
  real(r8) :: c_flx, n_flx, p_flx
  integer :: jj
  character(len=*),parameter :: subname = 'runbgc_ch4lake'
  associate(                                            &
    ...
    lid_nh4   => this%ch4lake_bgc_index%lid_nh4        , &  !position id of nh4
    lid_no3   => this%ch4lake_bgc_index%lid_no3        , &  !
    lid_o2    => this%ch4lake_bgc_index%lid_o2         , &  !
    nom_pools => this%ch4lake_bgc_index%nom_pools      , &  !number om pools
    nom_tot_elms=> this%ch4lake_bgc_index%nom_tot_elms , &
    nstvars   => this%ch4lake_bgc_index%nstvars        , &
    nprimvars => this%ch4lake_bgc_index%nprimvars      , &
    nreactions=> this%ch4lake_bgc_index%nreactions     , &
    lid_plant_minn_nh4  => this%ch4lake_bgc_index%lid_plant_minn_nh4       , &
    lid_plant_minn_no3  => this%ch4lake_bgc_index%lid_plant_minn_no3       , &
    lid_n2o_nit => this%ch4lake_bgc_index%lid_n2o_nit,&
    lid_no3_den => this%ch4lake_bgc_index%lid_no3_den,&
    lid_minp_soluble=> this%ch4lake_bgc_index%lid_minp_soluble, &
    cascade_matrix => this%cascade_matrix             , &
    cascade_matrixp=> this%cascade_matrixp            , &
    cascade_matrixd=> this%cascade_matrixd            , &
    ystates1 => this%ystates1                           &
  )
  this%ch4lake_bgc_index%debug = bgc_forc%debug
  if(this%ch4lake_bgc_index%debug)print*,'enter runbgc_ch4lake'
  this%rt_ar = rt_ar
  frc_c13 = safe_div(rt_ar_c13,rt_ar); frc_c14 = safe_div(rt_ar_c14,rt_ar)
  call bstatus%reset()

  !initialize state variables
  call this%init_states(this%ch4lake_bgc_index, bgc_forc)

  ystates0(:) = this%ystates0(:)


  !add all external input
   call this%add_ext_input(dtime, this%ch4lake_bgc_index, bgc_forc, &
  ...
  call this%calc_cascade_matrix(this%ch4lake_bgc_index, cascade_matrix, frc_c13, frc_c14)

  !run century decomposition, return decay rates, cascade matrix, potential hr
  call this%censom%run_decomp(is_surflit, this%ch4lake_bgc_index, dtime, ystates1(1:nom_tot_elms),&
  ...
  call this%nitden%run_nitden(this%ch4lake_bgc_index, bgc_forc, this%decompkf_eca, &
  ...
  call this%arenchyma_gas_transport(this%ch4lake_bgc_index, dtime)
  ...
  !if(this%ch4lake_bgc_index%debug)call this%checksum_cascade(this%ch4lake_bgc_index)
  if(this%use_c14)then
    call this%c14decay(this%ch4lake_bgc_index, dtime, ystates1)
  endif

  if(this%batch_mode)call this%sum_tot_store(nstvars, ystates1)
  ystatesf(:) = ystates1(:)
  end associate
  end subroutine runbgc_ch4lake
  !-------------------------------------------------------------------------------
  subroutine c14decay(this, ch4lake_bgc_index, dtime, ystates1)
  ...
  associate(                                &
    litr_beg =>  ch4lake_bgc_index%litr_beg, &
    Bm_beg =>  ch4lake_bgc_index%Bm_beg    , &
    som_beg =>  ch4lake_bgc_index%som_beg  , &
    dom_beg =>  ch4lake_bgc_index%dom_beg  , &
    pom_beg =>  ch4lake_bgc_index%pom_beg  , &
    wood_beg =>  ch4lake_bgc_index%wood_beg, &
    litr_end =>  ch4lake_bgc_index%litr_end, &
    som_end =>  ch4lake_bgc_index%som_end  , &
    Bm_end =>  ch4lake_bgc_index%Bm_end    , &
    dom_end =>  ch4lake_bgc_index%dom_end  , &
    pom_end => ch4lake_bgc_index%pom_end   , &
    wood_end =>  ch4lake_bgc_index%wood_end, &
    c14_loc=>  ch4lake_bgc_index%c14_loc   , &
    nelms => ch4lake_bgc_index%nelms         &
  )
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_cascade_matrix(this,ch4lake_bgc_index, cascade_matrix, frc_c13, frc_c14)
    !
    ! !DESCRIPTION:
    ...
    class(ch4lake_bgc_type)     , intent(in) :: this
    type(ch4lake_bgc_index_type)   , intent(in) :: ch4lake_bgc_index
    real(r8)                      , intent(inout)   :: cascade_matrix(ch4lake_bgc_index%nstvars, ch4lake_bgc_index%nreactions)
    ...
    associate(                                                             & !
         lid_autr_rt => ch4lake_bgc_index%lid_autr_rt                      , & !
         lid_o2    => ch4lake_bgc_index%lid_o2                             , & !
         lid_co2   => ch4lake_bgc_index%lid_co2                            , & !
         lid_c13_co2=> ch4lake_bgc_index%lid_c13_co2                       , & !
         lid_c14_co2=> ch4lake_bgc_index%lid_c14_co2                       , & !
         lid_nh4   => ch4lake_bgc_index%lid_nh4                            , & !
         lid_ch4   => ch4lake_bgc_index%lid_ch4                            , & !
         lid_ar    => ch4lake_bgc_index%lid_ar                             , & !
         lid_no3   => ch4lake_bgc_index%lid_no3                            , & !
         lid_n2o   => ch4lake_bgc_index%lid_n2o                            , & !
         lid_n2    => ch4lake_bgc_index%lid_n2                             , & !
         lid_co2_hr=> ch4lake_bgc_index%lid_co2_hr                         , & !
         lid_minn_nh4_immob => ch4lake_bgc_index%lid_minn_nh4_immob        , & !
         lid_minn_no3_immob => ch4lake_bgc_index%lid_minn_no3_immob        , & !
         lid_n2_paere => ch4lake_bgc_index%lid_n2_paere                    , & !
         lid_ch4_paere => ch4lake_bgc_index%lid_ch4_paere                  , & !
         lid_n2o_paere => ch4lake_bgc_index%lid_n2o_paere                  , & !
         lid_o2_paere => ch4lake_bgc_index%lid_o2_paere                    , & !
         lid_ar_paere => ch4lake_bgc_index%lid_ar_paere                    , & !
         lid_co2_paere => ch4lake_bgc_index%lid_co2_paere                  , & !
         lid_c13_co2_paere => ch4lake_bgc_index%lid_c13_co2_paere          , & !
         lid_c14_co2_paere => ch4lake_bgc_index%lid_c14_co2_paere          , & !
         lid_minp_soluble => ch4lake_bgc_index%lid_minp_soluble            , & !
         lid_minp_secondary => ch4lake_bgc_index%lid_minp_secondary        , & !
         lid_minp_occlude =>  ch4lake_bgc_index%lid_minp_occlude           , & !
         lid_plant_minp => ch4lake_bgc_index%lid_plant_minp                , & !
         lid_minp_immob => ch4lake_bgc_index%lid_minp_immob                , & !

         lid_autr_rt_reac=> ch4lake_bgc_index%lid_autr_rt_reac                 , & !
         lid_no3_den  => ch4lake_bgc_index%lid_no3_den                     , & !
         lid_plant_minn_nh4_up_reac=> ch4lake_bgc_index%lid_plant_minn_nh4_up_reac , & !
         lid_plant_minn_no3_up_reac=> ch4lake_bgc_index%lid_plant_minn_no3_up_reac , & !
         lid_plant_minn_nh4  => ch4lake_bgc_index%lid_plant_minn_nh4       , &
         lid_plant_minn_no3  => ch4lake_bgc_index%lid_plant_minn_no3       , &
         lid_minp_secondary_to_sol_occ_reac => ch4lake_bgc_index%lid_minp_secondary_to_sol_occ_reac    , & !
         lid_minp_soluble_to_secp_reac => ch4lake_bgc_index%lid_minp_soluble_to_secp_reac      , & !
         lid_plant_minp_up_reac => ch4lake_bgc_index%lid_plant_minp_up_reac, & !

         lid_n2_aren_reac => ch4lake_bgc_index%lid_n2_aren_reac            , & !
         lid_ch4_aren_reac=> ch4lake_bgc_index%lid_ch4_aren_reac           , & !
         lid_n2o_aren_reac=> ch4lake_bgc_index%lid_n2o_aren_reac           , & !
         lid_o2_aren_reac => ch4lake_bgc_index%lid_o2_aren_reac            , & !
         lid_ar_aren_reac => ch4lake_bgc_index%lid_ar_aren_reac            , & !
         lid_co2_aren_reac=> ch4lake_bgc_index%lid_co2_aren_reac           , & !
         lid_c13_co2_aren_reac=> ch4lake_bgc_index%lid_c13_co2_aren_reac   , & !
         lid_c14_co2_aren_reac=> ch4lake_bgc_index%lid_c14_co2_aren_reac     & !
         )
  ...
  !--------------------------------------------------------------------
  subroutine init_states(this, ch4lake_bgc_index, bgc_forc)
  ...
  associate(                           &
    ...
    lid_co2_hr => ch4lake_bgc_index%lid_co2_hr, &
    lid_n2o_nit => this%ch4lake_bgc_index%lid_n2o_nit,&
    lid_nh4_nit => this%ch4lake_bgc_index%lid_nh4_nit, &
    lid_no3_den => this%ch4lake_bgc_index%lid_no3_den,  &
    lid_minn_nh4_immob=> ch4lake_bgc_index%lid_minn_nh4_immob , &
    lid_minn_no3_immob => ch4lake_bgc_index%lid_minn_no3_immob, &
    lid_minp_immob => ch4lake_bgc_index%lid_minp_immob         &
  ...
  !--------------------------------------------------------------------
  subroutine add_ext_input(this, dtime, ch4lake_bgc_index, bgc_forc, c_inf, n_inf, p_inf)
  use ch4lakeBGCIndexType       , only : ch4lake_bgc_index_type
  use JarBgcForcType        , only : JarBGC_forc_type
  ...
  associate(                        &
    ...
    fwd =>   ch4lake_bgc_index%fwd, &
    lwd =>   ch4lake_bgc_index%lwd, &
    lid_totinput => ch4lake_bgc_index%lid_totinput, &
    litr_beg =>  ch4lake_bgc_index%litr_beg, &
    som_beg =>  ch4lake_bgc_index%som_beg, &
    Bm_beg  =>  ch4lake_bgc_index%Bm_beg, &
    dom_beg =>  ch4lake_bgc_index%dom_beg, &
  ...
  real(r8) :: pot_decomp(this%ch4lake_bgc_index%nom_pools)
  real(r8) :: rrates(this%ch4lake_bgc_index%nreactions)
  real(r8) :: p_dt(1:this%ch4lake_bgc_index%nprimvars)
  real(r8) :: d_dt(1:this%ch4lake_bgc_index%nprimvars)
  real(r8) :: dydt1(nstvars)
  real(r8) :: pscal(1:nprimvars)
  real(r8) :: rscal(1:this%ch4lake_bgc_index%nreactions)
  ...
  associate(                                                                                      &
    ...
    lid_nh4 => this%ch4lake_bgc_index%lid_nh4                                                    , &
    lid_no3 => this%ch4lake_bgc_index%lid_no3                                                    , &
    lid_co2_hr => this%ch4lake_bgc_index%lid_co2_hr                                              , &
    lid_cum_closs=> this%ch4lake_bgc_index%lid_cum_closs                                         , &
    som1    => this%ch4lake_bgc_index%som1                                                       , &
    som2    => this%ch4lake_bgc_index%som2                                                       , &
    som3    => this%ch4lake_bgc_index%som3                                                       , &
    ...
    lid_plant_minn_no3_pft=> this%ch4lake_bgc_index%lid_plant_minn_no3_pft                       , &
    lid_plant_minn_nh4_pft=> this%ch4lake_bgc_index%lid_plant_minn_nh4_pft                       , &
    lid_plant_minp_pft=> this%ch4lake_bgc_index%lid_plant_minp_pft                               , &
    lid_plant_minp    => this%ch4lake_bgc_index%lid_plant_minp                                   , &
    lid_plant_minn_nh4 => this%ch4lake_bgc_index%lid_plant_minn_nh4                              , &
    lid_plant_minn_no3 => this%ch4lake_bgc_index%lid_plant_minn_no3                              , &
    lid_minp_soluble => this%ch4lake_bgc_index%lid_minp_soluble                                  , &
    lid_minp_secondary => this%ch4lake_bgc_index%lid_minp_secondary                              , &
    lid_minp_soluble_to_secp_reac=> this%ch4lake_bgc_index%lid_minp_soluble_to_secp_reac         , &
    lid_autr_rt_reac => this%ch4lake_bgc_index%lid_autr_rt_reac                                  , &
    lid_nh4_nit_reac => this%ch4lake_bgc_index%lid_nh4_nit_reac                                  , &
    lid_no3_den_reac => this%ch4lake_bgc_index%lid_no3_den_reac                                  , &
    lid_plant_minn_nh4_up_reac => this%ch4lake_bgc_index%lid_plant_minn_nh4_up_reac              , &
    lid_plant_minn_no3_up_reac => this%ch4lake_bgc_index%lid_plant_minn_no3_up_reac              , &
    lid_plant_minp_up_reac => this%ch4lake_bgc_index%lid_plant_minp_up_reac                      , &
    lid_minp_secondary_to_sol_occ_reac=> this%ch4lake_bgc_index%lid_minp_secondary_to_sol_occ_reac, &
  ...
  call this%censom%calc_pot_min_np_flx(dtime, this%ch4lake_bgc_index,  ystate, this%k_decay,&
  ...
  this%competECA%debug=this%ch4lake_bgc_index%debug
  ...
  !--------------------------------------------------------------------
  subroutine arenchyma_gas_transport(this, ch4lake_bgc_index, dtime)
  use ch4lakeBGCIndexType       , only : ch4lake_bgc_index_type
  implicit none
  class(ch4lake_bgc_type)     , intent(inout) :: this
  type(ch4lake_bgc_index_type)  , intent(in) :: ch4lake_bgc_index
  ...
  this%ystates1(ch4lake_bgc_index%lid_o2_paere) = this%ystates1(j)-y0

  if( spinup_state == 0)then
    j = lid_n2; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(ch4lake_bgc_index%lid_n2_paere) = this%ystates1(j)-y0

    j = lid_ar; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(ch4lake_bgc_index%lid_ar_paere) = this%ystates1(j)-y0

    j = lid_ch4; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(ch4lake_bgc_index%lid_ch4_paere) = this%ystates1(j)-y0

    j = lid_co2; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(ch4lake_bgc_index%lid_co2_paere) = this%ystates1(j)-y0

    if(this%use_c13)then
      j = lid_c13_co2; y0=this%ystates1(j)
      call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
      this%ystates1(ch4lake_bgc_index%lid_c13_co2_paere) = this%ystates1(j)-y0
    ...
    if(this%use_c14)then
      ...
      call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
      this%ystates1(ch4lake_bgc_index%lid_c14_co2_paere) = this%ystates1(j)-y0

    endif
    j = lid_n2o; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(ch4lake_bgc_index%lid_n2o_paere) = this%ystates1(j)-y0
  endif
  end associate
  contains
  ...
  !--------------------------------------------------------------------
  subroutine sumup_cnp_msflx_ch4lake(this, ystates1, c_mass, n_mass, p_mass,c_flx,n_flx,p_flx, bstatus)
  ...
  associate(                        &
    ...
    lid_n2o_nit => this%ch4lake_bgc_index%lid_n2o_nit,&
    lid_no3_den => this%ch4lake_bgc_index%lid_no3_den,&
    lit1 =>  this%ch4lake_bgc_index%lit1, &
    lit2 =>  this%ch4lake_bgc_index%lit2, &
    lit3 =>  this%ch4lake_bgc_index%lit3, &
    cwd =>   this%ch4lake_bgc_index%cwd, &
    lwd =>   this%ch4lake_bgc_index%lwd, &
    fwd =>   this%ch4lake_bgc_index%fwd, &
    som1 =>  this%ch4lake_bgc_index%som1, &
    som2 =>  this%ch4lake_bgc_index%som2, &
    som3 =>  this%ch4lake_bgc_index%som3, &
    nelms => this%ch4lake_bgc_index%nelms, &
    ...
    lid_minp_soluble =>  this%ch4lake_bgc_index%lid_minp_soluble,  &
    lid_minp_secondary => this%ch4lake_bgc_index%lid_minp_secondary, &
    lid_minp_occlude => this%ch4lake_bgc_index%lid_minp_occlude, &
    lid_plant_minp => this%ch4lake_bgc_index%lid_plant_minp  &
  )
  if(present(bstatus)) &
    SHR_ASSERT_ALL((size(ystates1) == this%ch4lake_bgc_index%nstvars), errMsg(mod_filename,__LINE__),bstatus)
  ...
  end subroutine sumup_cnp_msflx_ch4lake
  ...
  associate(                        &
    ...
    n_loc=>  this%ch4lake_bgc_index%n_loc,&
    p_loc=>  this%ch4lake_bgc_index%p_loc,&
    lid_n2o => this%ch4lake_bgc_index%lid_n2o,&
    lid_n2 => this%ch4lake_bgc_index%lid_n2,&
    lit1 =>  this%ch4lake_bgc_index%lit1, &
    lit2 =>  this%ch4lake_bgc_index%lit2, &
    lit3 =>  this%ch4lake_bgc_index%lit3, &
    cwd =>   this%ch4lake_bgc_index%cwd, &
    ...
    lid_plant_minn_nh4 => this%ch4lake_bgc_index%lid_plant_minn_nh4, &
    lid_plant_minn_no3 => this%ch4lake_bgc_index%lid_plant_minn_no3, &
    lid_co2 => this%ch4lake_bgc_index%lid_co2, &
    lid_minp_soluble =>  this%ch4lake_bgc_index%lid_minp_soluble,  &
    lid_minp_secondary => this%ch4lake_bgc_index%lid_minp_secondary, &
    lid_minp_occlude => this%ch4lake_bgc_index%lid_minp_occlude, &
    lid_plant_minp => this%ch4lake_bgc_index%lid_plant_minp, &
  ...
  subroutine ode_adapt_ebbks1(me, y0, nprimeq, neq, t, dt, y)
    ...
    implicit none
    ! !ARGUMENTS:
    class(ch4lake_bgc_type),  intent(inout)  :: me
  ...
  !-------------------------------------------------------------------------------
  subroutine init_cold_ch4lake(this, nstvars, ystates)
  implicit none
  class(ch4lake_bgc_type)     , intent(inout) :: this
  integer , intent(in) :: nstvars
  ...
  end subroutine init_cold_ch4lake
  ...
  associate(                  &
    lid_totstore => this%ch4lake_bgc_index%lid_totstore , &
    lit1       => this%ch4lake_bgc_index%lit1        , &
    lit2       => this%ch4lake_bgc_index%lit2        , &
    lit3       => this%ch4lake_bgc_index%lit3        , &
    som1       => this%ch4lake_bgc_index%som1        , &
