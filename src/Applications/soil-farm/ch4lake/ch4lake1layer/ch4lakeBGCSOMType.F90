...
contains
  ...
  subroutine Init(this, ch4lake_bgc_index, biogeo_con, bstatus)
  ...
  call bstatus%reset()
  ncentpools = ch4lake_bgc_index%nom_pools
...
!------------------------------------------
  subroutine UpdateParas(this,ch4lake_bgc_index,  biogeo_con)
  !
  ...
  this%def_cn(ch4lake_bgc_index%lit1) = biogeo_con%init_cn_met * natomw/catomw;
  this%def_cn(ch4lake_bgc_index%lit2) = biogeo_con%init_cn_cel * natomw/catomw
  this%def_cn(ch4lake_bgc_index%lit3) = biogeo_con%init_cn_lig * natomw/catomw
  this%def_cn(ch4lake_bgc_index%cwd)  = biogeo_con%init_cn_cwd * natomw/catomw
  this%def_cn(ch4lake_bgc_index%lwd)  = biogeo_con%init_cn_lwd * natomw/catomw
  this%def_cn(ch4lake_bgc_index%fwd)  = biogeo_con%init_cn_fwd * natomw/catomw

  this%def_cn(ch4lake_bgc_index%som1) = biogeo_con%init_cn_som1 * natomw/catomw
  this%def_cn(ch4lake_bgc_index%som2) = biogeo_con%init_cn_som2 * natomw/catomw
  this%def_cn(ch4lake_bgc_index%som3) = biogeo_con%init_cn_som3 * natomw/catomw

  this%def_cp(ch4lake_bgc_index%lit1) = biogeo_con%init_cp_met * patomw/catomw
  this%def_cp(ch4lake_bgc_index%lit2) = biogeo_con%init_cp_cel * patomw/catomw
  this%def_cp(ch4lake_bgc_index%lit3) = biogeo_con%init_cp_lig * patomw/catomw
  this%def_cp(ch4lake_bgc_index%cwd)  = biogeo_con%init_cp_cwd * patomw/catomw
  this%def_cp(ch4lake_bgc_index%lwd)  = biogeo_con%init_cp_lwd * patomw/catomw
  this%def_cp(ch4lake_bgc_index%fwd)  = biogeo_con%init_cp_fwd * patomw/catomw

  this%def_cp(ch4lake_bgc_index%som1) = biogeo_con%init_cp_som1 * patomw/catomw
  this%def_cp(ch4lake_bgc_index%som2) = biogeo_con%init_cp_som2 * patomw/catomw
  this%def_cp(ch4lake_bgc_index%som3) = biogeo_con%init_cp_som3 * patomw/catomw

  this%use_c13=biogeo_con%use_c13
  this%use_c14=biogeo_con%use_c14

  if(this%use_c13)then
    this%def_cc13(ch4lake_bgc_index%lit1) = biogeo_con%init_cc13_met
    this%def_cc13(ch4lake_bgc_index%lit2) = biogeo_con%init_cc13_cel
    this%def_cc13(ch4lake_bgc_index%lit3) = biogeo_con%init_cc13_lig
    this%def_cc13(ch4lake_bgc_index%cwd)  = biogeo_con%init_cc13_cwd
    this%def_cc13(ch4lake_bgc_index%lwd)  = biogeo_con%init_cc13_lwd
    this%def_cc13(ch4lake_bgc_index%fwd)  = biogeo_con%init_cc13_fwd
    this%def_cc13(ch4lake_bgc_index%som1) = biogeo_con%init_cc13_som1
    this%def_cc13(ch4lake_bgc_index%som2) = biogeo_con%init_cc13_som2
    this%def_cc13(ch4lake_bgc_index%som3) = biogeo_con%init_cc13_som3
  endif

  if(this%use_c14)then
    this%def_cc14(ch4lake_bgc_index%lit1) = biogeo_con%init_cc14_met
    this%def_cc14(ch4lake_bgc_index%lit2) = biogeo_con%init_cc14_cel
    this%def_cc14(ch4lake_bgc_index%lit3) = biogeo_con%init_cc14_lig
    this%def_cc14(ch4lake_bgc_index%cwd)  = biogeo_con%init_cc14_cwd
    this%def_cc14(ch4lake_bgc_index%lwd)  = biogeo_con%init_cc14_lwd
    this%def_cc14(ch4lake_bgc_index%fwd)  = biogeo_con%init_cc14_fwd
    this%def_cc14(ch4lake_bgc_index%som1) = biogeo_con%init_cc14_som1
    this%def_cc14(ch4lake_bgc_index%som2) = biogeo_con%init_cc14_som2
    this%def_cc14(ch4lake_bgc_index%som3) = biogeo_con%init_cc14_som3
  endif

  end subroutine UpdateParas
!------------------------------------------

  subroutine run_decomp(this, is_surflit, ch4lake_bgc_index, dtime, ystates,&
  ...
  real(r8)                    , intent(inout) :: ystates(1:ch4lake_bgc_index%nom_tot_elms)
  ...
  associate(                                      &
    ...
    nom_tot_elms=> ch4lake_bgc_index%nom_tot_elms, &
  ...
  call this%calc_cnp_ratios(ch4lake_bgc_index, ystates, bstatus)
  if (bstatus%check_status())return
  call this%calc_som_decay_k(lay, ch4lake_bgc_index, decompkf_eca, k_decay)

  !calculate potential decay rates (mol C / s)
  call this%calc_som_decay_r(ch4lake_bgc_index, dtime, k_decay(1:ncentpools), &
  ...
  call this%calc_cascade_matrix(lay, ch4lake_bgc_index, pct_sand, pct_clay, alpha_n, alpha_p, cascade_matrix)

  !calculate potential respiration rates by summarizing all om decomposition pathways
  call this%calc_potential_aerobic_hr(ch4lake_bgc_index, pot_om_decay_rates, &
    cascade_matrix, pot_co2_hr, bstatus)
  end associate
  end subroutine run_decomp
!------------------------------------------

  subroutine calc_cascade_matrix(this, lay, ch4lake_bgc_index, pct_sand, pct_clay, alpha_n, alpha_p, cascade_matrix)
  !
  ! DESCRIPTION
  ...
  !-----------------------------------------------------------------------
  subroutine calc_potential_aerobic_hr(this, ch4lake_bgc_index, pot_decay_rates, &
    cascade_matrix, pot_co2_hr, bstatus)
    !
    ! DESCRIPTION:
    ...
    real(r8)                , intent(in) :: cascade_matrix(ch4lake_bgc_index%nstvars, ch4lake_bgc_index%nreactions)
    real(r8)                , intent(out):: pot_co2_hr
    type(betr_status_type)  , intent(out) :: bstatus
    ! !LOCAL VARIABLES:
  ...
  !-----------------------------------------------------------------------
  subroutine calc_cnp_ratios(this, ch4lake_bgc_index, ystates, bstatus)
  !
  ! DESCRIPTION
  ...
  class(CentSom_type), intent(inout) :: this
  type(ch4lake_bgc_index_type)   , intent(in) :: ch4lake_bgc_index
  real(r8)                      , intent(inout) :: ystates(ch4lake_bgc_index%nstvars)
  type(betr_status_type)      , intent(out) :: bstatus
  integer :: jj
  ...
  associate(                         &
    ...
    lit2  => ch4lake_bgc_index%lit2 , &
    lit3  => ch4lake_bgc_index%lit3 , &
    is_cenpool_som => ch4lake_bgc_index%is_cenpool_som, &
    ompoolnames => ch4lake_bgc_index%ompoolnames &
  )
  call bstatus%reset()
  !for om pools
  do jj = 1, ncentpools
    ...
    if(ch4lake_bgc_index%debug)then
 ...
 !     return
  ...
  !-------------------------------------------------------------------------------
  subroutine stoichiometry_fix(this, ch4lake_bgc_index,ystates)
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_som_decay_r(this, ch4lake_bgc_index, dtime, om_k_decay, om_pools, om_decay_rates)
    !
    ! !DESCRIPTION:
   ...
   type(ch4lake_bgc_index_type) , intent(in)    :: ch4lake_bgc_index
    ...
    real(r8)  , intent(in)    :: om_pools(ch4lake_bgc_index%nom_tot_elms)
  ...
  subroutine apply_spinupf(this, ch4lake_bgc_index, decompkf_eca, k_decay, spinup_scalar, spinup_flg)
  use ch4lakeBGCIndexType       , only : ch4lake_bgc_index_type
  use ch4lakeBGCDecompType      , only : DecompCent_type
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_som_decay_k(this, lay, ch4lake_bgc_index, decompkf_eca, k_decay)
...
!  print*,'scalar',t_scalar, w_scalar, o_scalar, depth_scalar
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_pot_min_np_flx(this, dtime, ch4lake_bgc_index, ystates, k_decay, cascade_matrix, &
  ...
  real(r8)                    , intent(in) :: dtime
  type(ch4lake_bgc_index_type) , intent(in) :: ch4lake_bgc_index
  real(r8)                    , intent(in) :: ystates(1:ch4lake_bgc_index%nom_tot_elms)
  ...
  !calculate potential decay rates (mol C / s)
  call this%calc_som_decay_r(ch4lake_bgc_index, dtime, k_decay(1:nom_pools), &
