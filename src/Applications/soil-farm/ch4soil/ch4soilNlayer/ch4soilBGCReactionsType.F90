  ...
  subroutine calc_bgc_reaction(this, bounds, col, lbj, ubj, num_soilc, filter_soilc, &
    ...
    do j = lbj, ubj
      do fc = 1, num_soilc
        ...
        call this%ch4soil(c,j)%runbgc(is_surflit, dtime, this%ch4soil_forc(c,j),nstates, &
...
!        if(.not. betrtracer_vars%debug)then
          ...
          call this%rm_ext_output(c, j, dtime, nstates, ystatesf, this%ch4soil_bgc_index,&
...
!        betrtracer_vars, tracerstate_vars,  'after bgcreact',betr_status)
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      this%ch4soil_forc(c,j)%depz   = col%z(c,j)            !depth of the soil
      this%ch4soil_forc(c,j)%dzsoi  = col%dz(c,j)            !soil thickness
