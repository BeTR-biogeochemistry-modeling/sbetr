...
#include "bshr_assert.h"
  ...
  character(len=*), private, parameter :: filename = &
       __FILE__
  public :: ecosys_plant_soilbgc_type

  type, extends(plant_soilbgc_type) :: &
    ecosys_plant_soilbgc_type
  ...
  end type ecosys_plant_soilbgc_type

  interface ecosys_plant_soilbgc_type
    module procedure constructor
  end interface ecosys_plant_soilbgc_type

  contains

  !-------------------------------------------------------------------------------
  type(ecosys_plant_soilbgc_type) function constructor()
  !
  ! !DESCRIPTION:
  ! create an object of type ecosys_plant_soilbgc_type.
  ! Right now it is purposely empty
   type(ecosys_plant_soilbgc_type), allocatable :: plants
   allocate(plants)
   constructor = plants
  ...
  implicit none
  ! !ARGUMENTS:
  class(ecosys_plant_soilbgc_type) , intent(inout) :: this
...
!  if(betrtracer_vars%debug)print*,'pft wcoltot',pft%npfts,sum(pft%wtcol(1:pft%npfts))
  ...
  !----------------------------------------------------------------------
end module ecosysPlantSoilBGCType
