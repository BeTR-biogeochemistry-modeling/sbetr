...
!
! !DESCRIPTION:
! This is an example generated from AppMaker for use in betr
!
! HISTORY:
! Created by Jinyun Tang, October 12, 2020
! !USES:
...
implicit none
  ...
  character(len=*), parameter :: mod_filename = &
    __FILE__
  public :: ecosys_bgc_reaction_type
  type, extends(bgc_reaction_type) :: &
    ecosys_bgc_reaction_type
  private
    integer :: parcol
    type(ecosys_bgc_type), pointer :: ecosys_bgc(:,:)
...
!----------------------------------------------------------------------
  ...
  !
  !DESCRIPTION
  !set up kinetic parameters needed by the model
  ...
  do j = lbj, ubj
    ...
    this%ecosys_forc(c_l,j)%Msurf_OM  = plantNutkinetics%minsurf_dom_compet_vr_col(c_l,j)
    this%ecosys_forc(c_l,j)%KM_OM_ref = plantNutkinetics%km_minsurf_dom_vr_col(c_l,j)
  enddo
...
!-------------------------------------------------------------------------------
  ...
  !create the models
  allocate(this%ecosys_bgc(bounds%begc:bounds%endc,lbj:ubj))
  !create model specific forcing data structure
  allocate(this%ecosys_forc(bounds%begc:bounds%endc,lbj:ubj))
  !initialize
  do j = lbj, ubj
    do c = bounds%begc, bounds%endc
      call this%ecosys_bgc(c,j)%Init(ecosys_para, batch_mode, bstatus)
      if(bstatus%check_status())return
...
!-------------------------------------------------------------------------------
  ...
  associate(                           &
    nelm    => this%ecosys_bgc_index%nelms,   &
    c_loc   => this%ecosys_bgc_index%c_loc,   &
    c13_loc => this%ecosys_bgc_index%c13_loc, &
    c14_loc => this%ecosys_bgc_index%c14_loc, &
    e_loc   => this%ecosys_bgc_index%e_loc    &
  )
  itemp_gwm     = 0
  itemp_g       = 0
  itemp_s       = 0
...
!-------------------------------------------------------------------------------
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      is_surflit=(j<=0)
      !do bgc reaction
      call this%ecosys_bgc(c,j)%runbgc(is_surflit, betr_time%delta_time, this%ecosys_forc(c,j), nstates, ystates0, ystatesf, betr_status)
...
!-------------------------------------------------------------------------------
  ...
  !
  ! DESCRIPTION
  ! retrieve fluxes after bgc update
  ! USES
  use tracerfluxType           , only : tracerflux_type
  use BeTR_decompMod           , only : betr_bounds_type
  use BeTRTracerType           , only : BeTRTracer_Type
...
!----------------------------------------------------------------------
  ...
  !
  ! DESCRIPTION
  ! retrieve fluxes from land to atmosphere
  ! USES
  use tracerfluxType           , only : tracerflux_type
  use BeTR_decompMod           , only : betr_bounds_type
  use BeTRTracerType           , only : BeTRTracer_Type
...
!------------------------------------------------------------------------------
  ...
  associate( &
    litr_beg =>  this%ecosys_bgc_index%litr_beg  , &
    litr_end =>  this%ecosys_bgc_index%litr_end  , &
    wood_beg =>  this%ecosys_bgc_index%wood_beg  , &
    wood_end =>  this%ecosys_bgc_index%wood_end  , &
    dom_beg =>  this%ecosys_bgc_index%dom_beg    , &
    dom_end =>  this%ecosys_bgc_index%dom_end    , &
    pom_beg =>  this%ecosys_bgc_index%pom_beg    , &
    pom_end =>  this%ecosys_bgc_index%pom_end    , &
    Bm_beg  =>  this%ecosys_bgc_index%Bm_beg     , &
    Bm_end  =>  this%ecosys_bgc_index%Bm_end     , &
  ...
  if ( betr_spinup_state == 0 ) then
    ...
    tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_co2x)) = &
      ystatesf(this%ecosys_bgc_index%lid_co2_paere)  - &
      ystates0(this%ecosys_bgc_index%lid_co2_paere)

    if(this%use_c13)then
      tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c13_co2x)) = &
        ystatesf(this%ecosys_bgc_index%lid_c13_co2_paere)  - &
        ystates0(this%ecosys_bgc_index%lid_c13_co2_paere)
    endif

    if(this%use_c14)then
      tracer_flx_parchm_vr(c,j,volatileid(betrtracer_vars%id_trc_c14_co2x)) = &
        ystatesf(this%ecosys_bgc_index%lid_c14_co2_paere)  - &
        ystates0(this%ecosys_bgc_index%lid_c14_co2_paere)
    endif
  ...
  tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_co2x ) = &
    ystatesf(this%ecosys_bgc_index%lid_co2) - &
    ystates0(this%ecosys_bgc_index%lid_co2)

  if(this%use_c13)then
    tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c13_co2x ) = &
      ystatesf(this%ecosys_bgc_index%lid_c13_co2) - &
      ystates0(this%ecosys_bgc_index%lid_c13_co2)
  endif

  if(this%use_c14)then
    tracer_flx_netpro_vr(c,j,betrtracer_vars%id_trc_c14_co2x ) = &
      ystatesf(this%ecosys_bgc_index%lid_c14_co2) - &
      ystates0(this%ecosys_bgc_index%lid_c14_co2)
  endif
...
!------------------------------------------------------------------------------
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      !litter
      this%ecosys_forc(c,j)%ystates(litr_beg:litr_end)= &
        tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_litr:betrtracer_vars%id_trc_end_litr)

      !wood
      this%ecosys_forc(c,j)%ystates(wood_beg:wood_end)= &
        tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_wood:betrtracer_vars%id_trc_end_wood)

      !dom
      this%ecosys_forc(c,j)%ystates(dom_beg:dom_end)= &
      tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_dom:betrtracer_vars%id_trc_end_dom)
      if(this%ecosys_forc(c,j)%ystates(dom_beg)<=tiny_cval)this%ecosys_forc(c,j)%ystates(dom_beg:dom_end)=0._r8

      !pom
      this%ecosys_forc(c,j)%ystates(pom_beg:pom_end)= &
        tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_pom:betrtracer_vars%id_trc_end_pom)
      if(this%ecosys_forc(c,j)%ystates(pom_beg)<=tiny_cval)this%ecosys_forc(c,j)%ystates(pom_beg:pom_end)=0._r8

      !microbial biomass
      this%ecosys_forc(c,j)%ystates(Bm_beg:Bm_end)= &
        tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_beg_Bm:betrtracer_vars%id_trc_end_Bm)
      if(this%ecosys_forc(c,j)%ystates(Bm_beg)<=tiny_cval)this%ecosys_forc(c,j)%ystates(Bm_beg:Bm_end)=0._r8

      !non-soluble phase of mineral p
      this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_n2) = &
         fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_n2))

      this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_o2) = &
        fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_o2))

      this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_ar) = &
        fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_ar))

      this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_co2)= &
        fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_co2x))

      if(this%use_c13)then
        this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_c13_co2)= &
          fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_c13_co2x))
      endif
      if(this%use_c14)then
        this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_c14_co2)= &
          fpmax(tracerstate_vars%tracer_conc_mobile_col(c, j, betrtracer_vars%id_trc_c14_co2x))
      endif

      this%ecosys_forc(c,j)%ystates(this%ecosys_bgc_index%lid_ch4)= &
      ...
      this%ecosys_forc(c,j)%depz   = col%z(c,j)            !depth of the soil
      this%ecosys_forc(c,j)%dzsoi  = col%dz(c,j)            !soil thickness
...
!------------------------------------------------------------------------------
  ...
  !  Kaff_CM              => ecosys_para%Kaff_CM                , &
  ! Kaff_EM              => ecosys_para%Kaff_EM                , &
  !  Kaff_BC              => ecosys_para%Kaff_BC                , &
  !  alpha_B2E            => ecosys_para%alpha_B2E              , &
  !  alpha_B2T            => ecosys_para%alpha_B2T              , &
    nelms                =>  this%ecosys_bgc_index%nelms         &
  )

  c_l=1
  do j = 1, ubj
    !KM_CM=aqu2bulkcef_mobile(c_l,j,id_trc_dom)*this%ecosys_forc(c_l,j)%KM_OM_ref*Kaff_CM
    !BMT=(tracer_conc_mobile(c_l,j,trcid_Bm)+tracer_conc_mobile(c_l,j,trcid_Bm+nelms))*alpha_B2T
    !Msurf=this%ecosys_forc(c_l,j)%Msurf_OM-tracer_conc_mobile(c_l,j,trcid_pom)
