...
#include "bshr_assert.h"
  ...
  use kecaBGCDecompType      , only : DecompCent_type
  use kecaBGCIndexType       , only : keca_bgc_index_type
  use kecaBGCNitDenType      , only : century_nitden_type
  use kecaBGCSOMType         , only : CentSom_type
  use kecaBGCCompetType      , only : Compet_KECA_type
  ...
  !Note:
  !Keeping keca_bgc_index as a private member is a workaround to call the ode solver
  !it increase the memory for each instance of keca_bgc_type, but enables
  !the ode function to be called by the ode solver

  type, extends(jar_model_type), public :: keca_bgc_type
    type(DecompCent_type),private        :: decompkf_eca
    type(century_nitden_type), private   :: nitden
    type(CentSom_type), private          :: censom
    type(Compet_KECA_type), public        :: competECA
    type(keca_bgc_index_type), private :: keca_bgc_index
  ...
  contains
    procedure, public  :: init          => init_keca
    procedure, public  :: runbgc        => runbgc_keca
    procedure, public  :: UpdateParas   => UpdateParas_keca
    procedure, public  :: sumup_cnp_msflx => sumup_cnp_msflx_keca
    procedure, public  :: getvarllen    => getvarllen_keca
    procedure, public  :: getvarlist    => getvarlist_keca
    procedure, public  :: init_cold     => init_cold_keca
  ...
  end type keca_bgc_type

  public :: create_jarmodel_keca
contains

  function create_jarmodel_keca()
  ! DESCRIPTION
  ! constructor
    implicit none
    class(keca_bgc_type), pointer :: create_jarmodel_keca
    class(keca_bgc_type), pointer :: bgc

    allocate(bgc)
    create_jarmodel_keca => bgc

  end function create_jarmodel_keca

  !-------------------------------------------------------------------------------
  function getvarllen_keca(this)result(ans)

  implicit none
  class(keca_bgc_type) , intent(inout) :: this
  integer :: ans

  ans = this%keca_bgc_index%nstvars

  end function getvarllen_keca
  !-------------------------------------------------------------------------------
  subroutine getvarlist_keca(this, nstvars, varnames, varunits, vartypes)
  implicit none
  class(keca_bgc_type) , intent(inout) :: this
  ...
  do n = 1, nstvars
    vartypes(n) = this%keca_bgc_index%vartypes(n)
    write(varnames(n),'(A)')trim(this%keca_bgc_index%varnames(n))
    write(varunits(n),'(A)')trim(this%keca_bgc_index%varunits(n))
  enddo
  end subroutine getvarlist_keca
  ...
  !-------------------------------------------------------------------------------
  subroutine UpdateParas_keca(this,  biogeo_con, bstatus)
  ...
  type is(keca_para_type)
    ...
    call this%censom%UpdateParas(this%keca_bgc_index, biogeo_con)
  ...
  end subroutine UpdateParas_keca
  !-------------------------------------------------------------------------------
  subroutine init_keca(this,  biogeo_con,  batch_mode, bstatus)
  ...
  type is(keca_para_type)
    call bstatus%reset()
    call this%keca_bgc_index%Init(biogeo_con%use_c13, biogeo_con%use_c14, &
    ...
    call this%InitAllocate(this%keca_bgc_index)

    call this%censom%Init(this%keca_bgc_index, biogeo_con, bstatus)
  ...
  class default
    call bstatus%reset()
    write(msg,'(A)')'Wrong parameter type passed in for init_keca in ' &
      // errMsg(mod_filename,__LINE__)
    call bstatus%set_msg(msg,err=-1)
    return
  end select
  end subroutine init_keca

  !-------------------------------------------------------------------------------

  subroutine InitAllocate(this, keca_bgc_index)
  use kecaBGCIndexType , only : keca_bgc_index_type
  use betr_varcon         , only : betr_maxpatch_pft, betr_max_soilorder
  implicit none
  class(keca_bgc_type)   , intent(inout) :: this
  ...
  associate(                                   &
    nom_pools=> keca_bgc_index%nom_pools,    &
    nstvars => keca_bgc_index%nstvars  ,     &
    nreactions => keca_bgc_index%nreactions, &
    nprimvars => keca_bgc_index%nprimvars    &
  )
  ...
  subroutine checksum_cascade(this, keca_bgc_index)

  use kecaBGCIndexType       , only : keca_bgc_index_type

  implicit none
  ! !ARGUMENTS:
  class(keca_bgc_type)     , intent(in) :: this
  type(keca_bgc_index_type)   , intent(in) :: keca_bgc_index
  ...
  associate(                                                   &
    ...
    lid_o2    => keca_bgc_index%lid_o2                     , & !
    lid_co2   => keca_bgc_index%lid_co2                    , & !
    lid_nh4   => keca_bgc_index%lid_nh4                    , & !
    lid_c14_co2=> keca_bgc_index%lid_c14_co2               , & !
    lid_c13_co2=> keca_bgc_index%lid_c13_co2               , & !
    lid_co2_hr => keca_bgc_index%lid_co2_hr                , &
    lid_minn_nh4_immob=> keca_bgc_index%lid_minn_nh4_immob , &
    lid_minp_immob => keca_bgc_index%lid_minp_immob        , &
    lid_minp_soluble=> keca_bgc_index%lid_minp_soluble     , &
    lit1_dek_reac => keca_bgc_index%lit1_dek_reac          , &
    lit2_dek_reac => keca_bgc_index%lit2_dek_reac          , &
    lit3_dek_reac => keca_bgc_index%lit3_dek_reac          , &
    som1_dek_reac => keca_bgc_index%som1_dek_reac          , &
    som2_dek_reac => keca_bgc_index%som2_dek_reac          , &
    som3_dek_reac => keca_bgc_index%som3_dek_reac          , &
    cwd_dek_reac => keca_bgc_index%cwd_dek_reac            , &
    lwd_dek_reac => keca_bgc_index%lwd_dek_reac            , &
    fwd_dek_reac => keca_bgc_index%fwd_dek_reac            , &
    lid_n2   => keca_bgc_index%lid_n2                      , & !
    lid_n2o   => keca_bgc_index%lid_n2o                    , & !
    lid_no3   => keca_bgc_index%lid_no3                    , & !
    lid_nh4_nit_reac => keca_bgc_index%lid_nh4_nit_reac    , & !
    lid_no3_den_reac => keca_bgc_index%lid_no3_den_reac      & !
  )
  print*,'checksum'
  ...
  !-------------------------------------------------------------------------------
  subroutine runbgc_keca(this,  is_surflit, dtime, bgc_forc, nstates, ystates0, ystatesf, bstatus)
  ...
  !local variables
  real(r8) :: pot_om_decay_rates(this%keca_bgc_index%nom_pools)
  real(r8) :: pot_co2_hr
  real(r8) :: pot_f_nit_mol_per_sec
  real(r8) :: n2_n2o_ratio_denit
  real(r8) :: yf(this%keca_bgc_index%nstvars)
  ...
  real(r8) :: c_flx, n_flx, p_flx
  integer :: jj
  character(len=*),parameter :: subname = 'runbgc_keca'
  associate(                                            &
    ...
    lid_nh4   => this%keca_bgc_index%lid_nh4        , &  !position id of nh4
    lid_no3   => this%keca_bgc_index%lid_no3        , &  !
    lid_o2    => this%keca_bgc_index%lid_o2         , &  !
    nom_pools => this%keca_bgc_index%nom_pools      , &  !number om pools
    nom_tot_elms=> this%keca_bgc_index%nom_tot_elms , &
    nstvars   => this%keca_bgc_index%nstvars        , &
    nprimvars => this%keca_bgc_index%nprimvars      , &
    nreactions=> this%keca_bgc_index%nreactions     , &
    lid_plant_minn_nh4  => this%keca_bgc_index%lid_plant_minn_nh4       , &
    lid_plant_minn_no3  => this%keca_bgc_index%lid_plant_minn_no3       , &
    lid_n2o_nit => this%keca_bgc_index%lid_n2o_nit,&
    lid_no3_den => this%keca_bgc_index%lid_no3_den,&
    lid_minp_soluble=> this%keca_bgc_index%lid_minp_soluble, &
    cascade_matrix => this%cascade_matrix             , &
    cascade_matrixp=> this%cascade_matrixp            , &
    cascade_matrixd=> this%cascade_matrixd            , &
    ystates1 => this%ystates1                           &
  )
  this%keca_bgc_index%debug = bgc_forc%debug
  if(this%keca_bgc_index%debug)print*,'enter runbgc_keca'
  this%rt_ar = rt_ar
  frc_c13 = safe_div(rt_ar_c13,rt_ar); frc_c14 = safe_div(rt_ar_c14,rt_ar)
  call bstatus%reset()

  !initialize state variables
  call this%init_states(this%keca_bgc_index, bgc_forc)

  ystates0(:) = this%ystates0(:)


  !add all external input
   call this%add_ext_input(dtime, this%keca_bgc_index, bgc_forc, &
  ...
  call this%calc_cascade_matrix(this%keca_bgc_index, cascade_matrix, frc_c13, frc_c14)

  !run century decomposition, return decay rates, cascade matrix, potential hr
  call this%censom%run_decomp(is_surflit, this%keca_bgc_index, dtime, ystates1(1:nom_tot_elms),&
  ...
  call this%nitden%run_nitden(this%keca_bgc_index, bgc_forc, this%decompkf_eca, &
  ...
  call this%arenchyma_gas_transport(this%keca_bgc_index, dtime)
  ...
  !if(this%keca_bgc_index%debug)call this%checksum_cascade(this%keca_bgc_index)
  if(this%use_c14)then
    call this%c14decay(this%keca_bgc_index, dtime, ystates1)
  endif

  if(this%batch_mode)call this%sum_tot_store(nstvars, ystates1)
  ystatesf(:) = ystates1(:)
  end associate
  end subroutine runbgc_keca
  !-------------------------------------------------------------------------------
  subroutine c14decay(this, keca_bgc_index, dtime, ystates1)
  ...
  associate(                                &
    litr_beg =>  keca_bgc_index%litr_beg, &
    Bm_beg =>  keca_bgc_index%Bm_beg    , &
    som_beg =>  keca_bgc_index%som_beg  , &
    dom_beg =>  keca_bgc_index%dom_beg  , &
    pom_beg =>  keca_bgc_index%pom_beg  , &
    wood_beg =>  keca_bgc_index%wood_beg, &
    litr_end =>  keca_bgc_index%litr_end, &
    som_end =>  keca_bgc_index%som_end  , &
    Bm_end =>  keca_bgc_index%Bm_end    , &
    dom_end =>  keca_bgc_index%dom_end  , &
    pom_end => keca_bgc_index%pom_end   , &
    wood_end =>  keca_bgc_index%wood_end, &
    c14_loc=>  keca_bgc_index%c14_loc   , &
    nelms => keca_bgc_index%nelms         &
  )
  ...
  !-------------------------------------------------------------------------------
  subroutine calc_cascade_matrix(this,keca_bgc_index, cascade_matrix, frc_c13, frc_c14)
    !
    ! !DESCRIPTION:
    ...
    class(keca_bgc_type)     , intent(in) :: this
    type(keca_bgc_index_type)   , intent(in) :: keca_bgc_index
    real(r8)                      , intent(inout)   :: cascade_matrix(keca_bgc_index%nstvars, keca_bgc_index%nreactions)
    ...
    associate(                                                             & !
         lid_autr_rt => keca_bgc_index%lid_autr_rt                      , & !
         lid_o2    => keca_bgc_index%lid_o2                             , & !
         lid_co2   => keca_bgc_index%lid_co2                            , & !
         lid_c13_co2=> keca_bgc_index%lid_c13_co2                       , & !
         lid_c14_co2=> keca_bgc_index%lid_c14_co2                       , & !
         lid_nh4   => keca_bgc_index%lid_nh4                            , & !
         lid_ch4   => keca_bgc_index%lid_ch4                            , & !
         lid_ar    => keca_bgc_index%lid_ar                             , & !
         lid_no3   => keca_bgc_index%lid_no3                            , & !
         lid_n2o   => keca_bgc_index%lid_n2o                            , & !
         lid_n2    => keca_bgc_index%lid_n2                             , & !
         lid_co2_hr=> keca_bgc_index%lid_co2_hr                         , & !
         lid_minn_nh4_immob => keca_bgc_index%lid_minn_nh4_immob        , & !
         lid_minn_no3_immob => keca_bgc_index%lid_minn_no3_immob        , & !
         lid_n2_paere => keca_bgc_index%lid_n2_paere                    , & !
         lid_ch4_paere => keca_bgc_index%lid_ch4_paere                  , & !
         lid_n2o_paere => keca_bgc_index%lid_n2o_paere                  , & !
         lid_o2_paere => keca_bgc_index%lid_o2_paere                    , & !
         lid_ar_paere => keca_bgc_index%lid_ar_paere                    , & !
         lid_co2_paere => keca_bgc_index%lid_co2_paere                  , & !
         lid_c13_co2_paere => keca_bgc_index%lid_c13_co2_paere          , & !
         lid_c14_co2_paere => keca_bgc_index%lid_c14_co2_paere          , & !
         lid_minp_soluble => keca_bgc_index%lid_minp_soluble            , & !
         lid_minp_secondary => keca_bgc_index%lid_minp_secondary        , & !
         lid_minp_occlude =>  keca_bgc_index%lid_minp_occlude           , & !
         lid_plant_minp => keca_bgc_index%lid_plant_minp                , & !
         lid_minp_immob => keca_bgc_index%lid_minp_immob                , & !

         lid_autr_rt_reac=> keca_bgc_index%lid_autr_rt_reac                 , & !
         lid_no3_den  => keca_bgc_index%lid_no3_den                     , & !
         lid_plant_minn_nh4_up_reac=> keca_bgc_index%lid_plant_minn_nh4_up_reac , & !
         lid_plant_minn_no3_up_reac=> keca_bgc_index%lid_plant_minn_no3_up_reac , & !
         lid_plant_minn_nh4  => keca_bgc_index%lid_plant_minn_nh4       , &
         lid_plant_minn_no3  => keca_bgc_index%lid_plant_minn_no3       , &
         lid_minp_secondary_to_sol_occ_reac => keca_bgc_index%lid_minp_secondary_to_sol_occ_reac    , & !
         lid_minp_soluble_to_secp_reac => keca_bgc_index%lid_minp_soluble_to_secp_reac      , & !
         lid_plant_minp_up_reac => keca_bgc_index%lid_plant_minp_up_reac, & !

         lid_n2_aren_reac => keca_bgc_index%lid_n2_aren_reac            , & !
         lid_ch4_aren_reac=> keca_bgc_index%lid_ch4_aren_reac           , & !
         lid_n2o_aren_reac=> keca_bgc_index%lid_n2o_aren_reac           , & !
         lid_o2_aren_reac => keca_bgc_index%lid_o2_aren_reac            , & !
         lid_ar_aren_reac => keca_bgc_index%lid_ar_aren_reac            , & !
         lid_co2_aren_reac=> keca_bgc_index%lid_co2_aren_reac           , & !
         lid_c13_co2_aren_reac=> keca_bgc_index%lid_c13_co2_aren_reac   , & !
         lid_c14_co2_aren_reac=> keca_bgc_index%lid_c14_co2_aren_reac     & !
         )
  ...
  !--------------------------------------------------------------------
  subroutine init_states(this, keca_bgc_index, bgc_forc)
  ...
  associate(                           &
    ...
    lid_co2_hr => keca_bgc_index%lid_co2_hr, &
    lid_n2o_nit => this%keca_bgc_index%lid_n2o_nit,&
    lid_nh4_nit => this%keca_bgc_index%lid_nh4_nit, &
    lid_no3_den => this%keca_bgc_index%lid_no3_den,  &
    lid_minn_nh4_immob=> keca_bgc_index%lid_minn_nh4_immob , &
    lid_minn_no3_immob => keca_bgc_index%lid_minn_no3_immob, &
    lid_minp_immob => keca_bgc_index%lid_minp_immob         &
  ...
  !--------------------------------------------------------------------
  subroutine add_ext_input(this, dtime, keca_bgc_index, bgc_forc, c_inf, n_inf, p_inf)
  use kecaBGCIndexType       , only : keca_bgc_index_type
  use JarBgcForcType        , only : JarBGC_forc_type
  ...
  associate(                        &
    ...
    fwd =>   keca_bgc_index%fwd, &
    lwd =>   keca_bgc_index%lwd, &
    lid_totinput => keca_bgc_index%lid_totinput, &
    litr_beg =>  keca_bgc_index%litr_beg, &
    som_beg =>  keca_bgc_index%som_beg, &
    Bm_beg  =>  keca_bgc_index%Bm_beg, &
    dom_beg =>  keca_bgc_index%dom_beg, &
  ...
  real(r8) :: pot_decomp(this%keca_bgc_index%nom_pools)
  real(r8) :: rrates(this%keca_bgc_index%nreactions)
  real(r8) :: p_dt(1:this%keca_bgc_index%nprimvars)
  real(r8) :: d_dt(1:this%keca_bgc_index%nprimvars)
  real(r8) :: dydt1(nstvars)
  real(r8) :: pscal(1:nprimvars)
  real(r8) :: rscal(1:this%keca_bgc_index%nreactions)
  ...
  associate(                                                                                      &
    ...
    lid_nh4 => this%keca_bgc_index%lid_nh4                                                    , &
    lid_no3 => this%keca_bgc_index%lid_no3                                                    , &
    lid_co2_hr => this%keca_bgc_index%lid_co2_hr                                              , &
    lid_cum_closs=> this%keca_bgc_index%lid_cum_closs                                         , &
    som1    => this%keca_bgc_index%som1                                                       , &
    som2    => this%keca_bgc_index%som2                                                       , &
    som3    => this%keca_bgc_index%som3                                                       , &
    ...
    lid_plant_minn_no3_pft=> this%keca_bgc_index%lid_plant_minn_no3_pft                       , &
    lid_plant_minn_nh4_pft=> this%keca_bgc_index%lid_plant_minn_nh4_pft                       , &
    lid_plant_minp_pft=> this%keca_bgc_index%lid_plant_minp_pft                               , &
    lid_plant_minp    => this%keca_bgc_index%lid_plant_minp                                   , &
    lid_plant_minn_nh4 => this%keca_bgc_index%lid_plant_minn_nh4                              , &
    lid_plant_minn_no3 => this%keca_bgc_index%lid_plant_minn_no3                              , &
    lid_minp_soluble => this%keca_bgc_index%lid_minp_soluble                                  , &
    lid_minp_secondary => this%keca_bgc_index%lid_minp_secondary                              , &
    lid_minp_soluble_to_secp_reac=> this%keca_bgc_index%lid_minp_soluble_to_secp_reac         , &
    lid_autr_rt_reac => this%keca_bgc_index%lid_autr_rt_reac                                  , &
    lid_nh4_nit_reac => this%keca_bgc_index%lid_nh4_nit_reac                                  , &
    lid_no3_den_reac => this%keca_bgc_index%lid_no3_den_reac                                  , &
    lid_plant_minn_nh4_up_reac => this%keca_bgc_index%lid_plant_minn_nh4_up_reac              , &
    lid_plant_minn_no3_up_reac => this%keca_bgc_index%lid_plant_minn_no3_up_reac              , &
    lid_plant_minp_up_reac => this%keca_bgc_index%lid_plant_minp_up_reac                      , &
    lid_minp_secondary_to_sol_occ_reac=> this%keca_bgc_index%lid_minp_secondary_to_sol_occ_reac, &
    lid_supp_minp => this%keca_bgc_index%lid_supp_minp                                         , &
    lid_supp_minn => this%keca_bgc_index%lid_supp_minn                                           &
  )
  ...
  call this%censom%calc_pot_min_np_flx(dtime, this%keca_bgc_index,  ystate, this%k_decay,&
  ...
  this%competECA%debug=this%keca_bgc_index%debug
  ...
  !--------------------------------------------------------------------
  subroutine arenchyma_gas_transport(this, keca_bgc_index, dtime)
  use kecaBGCIndexType       , only : keca_bgc_index_type
  implicit none
  class(keca_bgc_type)     , intent(inout) :: this
  type(keca_bgc_index_type)  , intent(in) :: keca_bgc_index
  ...
  this%ystates1(keca_bgc_index%lid_o2_paere) = this%ystates1(j)-y0

  if( spinup_state == 0)then
    j = lid_n2; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(keca_bgc_index%lid_n2_paere) = this%ystates1(j)-y0

    j = lid_ar; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(keca_bgc_index%lid_ar_paere) = this%ystates1(j)-y0

    j = lid_ch4; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(keca_bgc_index%lid_ch4_paere) = this%ystates1(j)-y0

    j = lid_co2; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(keca_bgc_index%lid_co2_paere) = this%ystates1(j)-y0

    if(this%use_c13)then
      j = lid_c13_co2; y0=this%ystates1(j)
      call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
      this%ystates1(keca_bgc_index%lid_c13_co2_paere) = this%ystates1(j)-y0
    ...
    if(this%use_c14)then
      ...
      call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
      this%ystates1(keca_bgc_index%lid_c14_co2_paere) = this%ystates1(j)-y0

    endif
    j = lid_n2o; y0=this%ystates1(j)
    call exp_ode_int(dtime, this%scal_f(j), this%conv_f(j), this%conc_f(j), this%ystates1(j))
    this%ystates1(keca_bgc_index%lid_n2o_paere) = this%ystates1(j)-y0
  endif
  end associate
  contains
  ...
  !--------------------------------------------------------------------
  subroutine sumup_cnp_msflx_keca(this, ystates1, c_mass, n_mass, p_mass,c_flx,n_flx,p_flx, bstatus)
  ...
  associate(                        &
    ...
    lid_n2o_nit => this%keca_bgc_index%lid_n2o_nit,&
    lid_no3_den => this%keca_bgc_index%lid_no3_den,&
    lit1 =>  this%keca_bgc_index%lit1, &
    lit2 =>  this%keca_bgc_index%lit2, &
    lit3 =>  this%keca_bgc_index%lit3, &
    cwd =>   this%keca_bgc_index%cwd, &
    lwd =>   this%keca_bgc_index%lwd, &
    fwd =>   this%keca_bgc_index%fwd, &
    som1 =>  this%keca_bgc_index%som1, &
    som2 =>  this%keca_bgc_index%som2, &
    som3 =>  this%keca_bgc_index%som3, &
    nelms => this%keca_bgc_index%nelms, &
    ...
    lid_minp_soluble =>  this%keca_bgc_index%lid_minp_soluble,  &
    lid_minp_secondary => this%keca_bgc_index%lid_minp_secondary, &
    lid_minp_occlude => this%keca_bgc_index%lid_minp_occlude, &
    lid_plant_minp => this%keca_bgc_index%lid_plant_minp  &
  )
  if(present(bstatus)) &
    SHR_ASSERT_ALL((size(ystates1) == this%keca_bgc_index%nstvars), errMsg(mod_filename,__LINE__),bstatus)
  ...
  end subroutine sumup_cnp_msflx_keca
  ...
  associate(                        &
    ...
    n_loc=>  this%keca_bgc_index%n_loc,&
    p_loc=>  this%keca_bgc_index%p_loc,&
    lid_n2o => this%keca_bgc_index%lid_n2o,&
    lid_n2 => this%keca_bgc_index%lid_n2,&
    lit1 =>  this%keca_bgc_index%lit1, &
    lit2 =>  this%keca_bgc_index%lit2, &
    lit3 =>  this%keca_bgc_index%lit3, &
    cwd =>   this%keca_bgc_index%cwd, &
    ...
    lid_plant_minn_nh4 => this%keca_bgc_index%lid_plant_minn_nh4, &
    lid_plant_minn_no3 => this%keca_bgc_index%lid_plant_minn_no3, &
    lid_co2 => this%keca_bgc_index%lid_co2, &
    lid_minp_soluble =>  this%keca_bgc_index%lid_minp_soluble,  &
    lid_minp_secondary => this%keca_bgc_index%lid_minp_secondary, &
    lid_minp_occlude => this%keca_bgc_index%lid_minp_occlude, &
    lid_plant_minp => this%keca_bgc_index%lid_plant_minp, &
  ...
  subroutine ode_adapt_ebbks1(me, y0, nprimeq, neq, t, dt, y)
    ...
    implicit none
    ! !ARGUMENTS:
    class(keca_bgc_type),  intent(inout)  :: me
  ...
  !-------------------------------------------------------------------------------
  subroutine init_cold_keca(this, nstvars, ystates)
  implicit none
  class(keca_bgc_type)     , intent(inout) :: this
  integer , intent(in) :: nstvars
  ...
  associate(                  &
    lid_totstore => this%keca_bgc_index%lid_totstore , &
    lit1       => this%keca_bgc_index%lit1        , &
    lit2       => this%keca_bgc_index%lit2        , &
    lit3       => this%keca_bgc_index%lit3        , &
    som1       => this%keca_bgc_index%som1        , &
