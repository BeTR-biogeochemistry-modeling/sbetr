...
#include "bshr_assert.h"
  ...
  character(len=*), private, parameter :: filename = &
       __FILE__
  public :: keca_plant_soilbgc_type
  ...
  end type keca_plant_soilbgc_type

  interface keca_plant_soilbgc_type
    module procedure constructor
  end interface keca_plant_soilbgc_type

  contains

  !-------------------------------------------------------------------------------
  type(keca_plant_soilbgc_type) function constructor()
  !
  ! !DESCRIPTION:
  ! create an object of type keca_plant_soilbgc_type.
  ! Right now it is purposely empty
   type(keca_plant_soilbgc_type), allocatable :: plants
   allocate(plants)
   constructor = plants
  ...
  implicit none
  ! !ARGUMENTS:
  class(keca_plant_soilbgc_type) , intent(inout) :: this
...
!  if(betrtracer_vars%debug)print*,'pft wcoltot',pft%npfts,sum(pft%wtcol(1:pft%npfts))
  ...
  !----------------------------------------------------------------------
end module kecaPlantSoilBGCType
