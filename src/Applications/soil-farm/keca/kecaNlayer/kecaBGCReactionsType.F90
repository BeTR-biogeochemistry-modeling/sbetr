...
contains
  ...
  if(betr_spinup_state/=0)then
    adv_scalar(tracers%id_trc_Bm)  = keca_para%spinup_factor(7)
    adv_scalar(tracers%id_trc_som) = keca_para%spinup_factor(8)*latacc
    adv_scalar(tracers%id_trc_pom)=keca_para%spinup_factor(9)  *latacc
    difu_scalar(tracers%id_trc_Bm)  = keca_para%spinup_factor(7)
    difu_scalar(tracers%id_trc_som) = keca_para%spinup_factor(8)*latacc
    difu_scalar(tracers%id_trc_pom)=keca_para%spinup_factor(9)  *latacc
  endif
    ...
    do j = lbj, ubj
      do fc = 1, num_soilc
        ...
        this%keca_forc(c,j)%tmic_opt = this%keca_forc(c,j)%tmic_opt + &
           betr_time%delta_time/keca_para%tau30*(this%keca_forc(c,j)%temp-this%keca_forc(c,j)%tmic_opt)
        call this%keca(c,j)%runbgc(is_surflit, betr_time%delta_time, this%keca_forc(c,j),nstates, &
...
!        if(.not. betrtracer_vars%debug)then
          ...
          call this%rm_ext_output(c, j, betr_time%delta_time, nstates, ystatesf, this%keca_bgc_index,&
...
!     biogeo_flux%n14flux_vars%smin_nh4_qdrain_col(c) = tracer_flx_drain_col(c,id_trc_nh3x) * natomw
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      ...
      this%keca_forc(c,j)%depz   = biophysforc%z(c,j)            !depth of the soil
      this%keca_forc(c,j)%dzsoi  = biophysforc%dz(c,j)            !soil thickness
  ...
  do j = lbj, ubj
    do fc = 1, num_soilc
      c = filter_soilc(fc)
      this%keca_forc(c,j)%rt_ar  = biophysforc%c12flx%rt_vr_col(c,j)            !root autotrophic respiration, mol CO2/m3/s
    enddo
  enddo
  end select
  end associate
