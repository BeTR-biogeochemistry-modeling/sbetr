...
implicit none
 ...
 subroutine InitAllocate(this, bounds)
   ...
   use betr_varcon    , only : spval  => bspva
   !
   ! !ARGUMENTS:
   class(DecompECA_Type) :: this
   type(bounds_type), intent(in) :: bounds
   ...
   ubj = bounds%ubj;
   allocate(this%t_scalar_col     (begc:endc,1:ubj)); this%t_scalar_col (:,:)=spval
   allocate(this%w_scalar_col     (begc:endc,1:ubj)); this%w_scalar_col (:,:)=spval
   allocate(this%o_scalar_col     (begc:endc,1:ubj)); this%o_scalar_col (:,:)=spval
   allocate(this%depth_scalar_col (begc:endc,1:ubj)); this%depth_scalar_col(:,:)=spval
